...
function equipmentPage(title) {
  const vendorOptions = state.vendors.map((x) => `<option value="${x.id}">${esc(x.vendor_code)} · ${esc(x.legal_name)}</option>`).join("");
  return `${projectHeader()}${can(PERMISSIONS.CREATE) ? `<section class="hp-card"><h3>Add ${esc(title)} item</h3><p class="muted">Create the equipment requirement without leaving this project workspace.</p><form id="projectEquipmentForm" class="hp-form"><label>Equipment<input name="equipment_name" required></label><label>Make / model<input name="make_model"></label><label>Vendor<select name="vendor_id"><option value="">Unassigned</option>${vendorOptions}</select></label><label>Quantity<input name="quantity" type="number" min=".001" step=".001" value="1" required></label><label>Vendor unit price<input name="vendor_unit_cost" type="number" min="0" step=".01" value="0"></label><label>Landed unit cost<input name="landed_unit_cost" type="number" min="0" step=".01" value="0"></label><label>Hospital unit selling price<input name="client_unit_price" type="number" min="0" step=".01" value="0"></label><label class="wide">Specification<textarea name="specification"></textarea></label><button class="btn">Add equipment</button></form></section>` : ""}<section class="hp-card"><h3>${esc(title)}</h3><p class="muted">Project-scoped equipment and procurement register.</p>${table(["Equipment", "Vendor", "Quantity", "Landed cost", "Hospital price", "Margin", "Status"], state.items.map((x) => `<tr><td><strong>${esc(x.equipment_name)}</strong><br><small>${esc(x.make_model || "")}</small></td><td>${esc(x.hospital_vendors?.legal_name || "Unassigned")}</td><td>${Number(x.quantity) || 0}</td><td>${money(x.total_vendor_cost)}</td><td>${money(x.total_client_price)}</td><td>${money(x.margin_amount)}</td><td>${esc(label(x.status))}</td></tr>`).join(""), "No equipment requirements")}</section>`;
}
...
function bind() {
  ...
  document.getElementById("projectEquipmentForm")?.addEventListener("submit", async (event) => { event.preventDefault();try{const payload=values(event.currentTarget);payload.project_id=projectId;for(const key of ["quantity","vendor_unit_cost","landed_unit_cost","client_unit_price"])payload[key]=Number(payload[key]||0);const {data,error}=await db.from("hospital_procurement_items").insert(payload).select("id").single();if(error)throw error;await logAuditEvent("hospital_equipment_item_created",{moduleCode:MODULES.HOSPITAL_PROJECTS,entityType:"hospital_procurement_items",entityId:data?.id,afterData:payload});showToast("Equipment item added.",TOAST_TYPES.SUCCESS);await loadView();render();}catch(error){showToast(error.message,TOAST_TYPES.ERROR);} });
