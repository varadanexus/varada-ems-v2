...
export function renderWalletManagementPage(billing = {}) {
  return `<section class="wp-route-page wp-billing-page">
    ...
    <section class="wp-wallet-command"><div class="wp-wallet-command-copy"><span class="wp-wallet-command-icon" aria-hidden="true">$</span><div><span class="wp-card-eyebrow">Prepaid service wallet</span><h2>Your money, usage and payments—clearly organised</h2><p>Recharge only what you need. Spendable balance, reserved funds, taxes and gateway fees stay separate and auditable.</p></div></div><div class="wp-wallet-command-rate"><span>Your Varada message fee</span><strong data-wallet-page-rate>Loading…</strong><small>per incoming or outgoing message</small></div></section>
...
export function renderPaygBillingOverview(billing = {}) {
  ...
  return `<section class="wp-route-page wp-billing-page">
    ...
    <section class="wp-billing-overview-metrics"><article><span>Service wallet</span><strong>${active ? 'Active' : 'Setup pending'}</strong><small>${active ? 'Usage charging enabled' : 'No message charging yet'}</small></article><article><span>Message rate</span><strong>Workspace-specific</strong><small>View the wallet for the current verified rate</small></article><article><span>Payment ledger</span><strong>Auditable</strong><small>Recharges, balance movements and usage</small></article><article><span>Financial archive</span><strong>${invoices + credits}</strong><small>${invoices} invoices · ${credits} credit notes</small></article></section>
