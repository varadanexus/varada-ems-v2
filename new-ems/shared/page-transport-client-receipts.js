...
async function openDetailsModal(receiptId) {
  ...
  if (!host) return;
  const billType = details.transport_client_bills?.billing_type || "NON_GST";
  const billTotal = billType === "GST"
    ? Number(details.transport_client_bills?.invoice_total || details.transport_client_bills?.net_receivable || 0)
    : Number(details.transport_client_bills?.net_receivable || 0);
  host.innerHTML = `<div class="pay-detail-grid"><div class="pay-detail-box"><label>Receipt No</label><strong>${escapeHtml(details.receipt_no || "—")}</strong></div><div class="pay-detail-box"><label>Client</label><strong>${escapeHtml(resolveClientLabel(details))}</strong></div><div class="pay-detail-box"><label>Bill No</label><strong>${escapeHtml(details.transport_client_bills?.bill_no || "—")}</strong></div><div class="pay-detail-box"><label>Billing Type</label><strong>${escapeHtml(billType)}</strong></div><div class="pay-detail-box"><label>Receipt Date</label><strong>${escapeHtml(details.receipt_date || "—")}</strong></div><div class="pay-detail-box"><label>Amount Received</label><strong>${formatMoney(details.amount_received)}</strong></div><div class="pay-detail-box"><label>Payment Mode</label><strong>${escapeHtml(details.payment_mode || "—")}</strong></div><div class="pay-detail-box"><label>Status</label><strong>${escapeHtml(details.status || "—")}</strong></div><div class="pay-detail-box"><label>Reference No</label><strong>${escapeHtml(details.reference_no || "—")}</strong></div><div class="pay-detail-box"><label>Remarks</label><strong>${escapeHtml(details.remarks || "—")}</strong></div><div class="pay-detail-box"><label>Created At</label><strong>${formatDateTime(details.created_at)}</strong></div><div class="pay-detail-box"><label>Bill Total</label><strong>${formatMoney(billTotal)}</strong></div></div>`;
