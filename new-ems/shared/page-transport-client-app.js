...
const TRIP_SORTABLE_COLUMNS = new Set(["trip_no", "trip_date", "quantity_mt", "client_gross_amount", "status"]);
...
async function loadSection() {
  ...
  try {
    ...
    } else if (PAGE_STATE.activeSection === "trips" || !PAGE_STATE.trips.length) {
    ...
    if (PAGE_STATE.activeSection === "bills") {
    ...
    if (PAGE_STATE.activeSection === "gst") {
    ...
    if (PAGE_STATE.activeSection === "credit-notes") {
    ...
    if (PAGE_STATE.activeSection === "receipts") {
      ...
      <div class="portal-brand-block">
        <div class="portal-brand-lockup">
          ...
          <div class="portal-brand-copy">
            ...
            <div class="portal-brand-module">Client Portal</div>
          </div>
        </div>
      </div>
      <nav class="nav-root">
        <div class="nav-section">
          ...
          <div class="portal-account-card">
            <div class="portal-account-eyebrow">Active Client</div>
            <strong class="portal-account-name">${escapeHtml(activeClient()?.name || "-")}</strong>
            <div class="form-row portal-account-selector-row">
              <select id="clientSelector" style="width:100%;">
                ${PAGE_STATE.clients.map((c) => `<option value="${c.id}" ${c.id === PAGE_STATE.activeClientId ? "selected" : ""}>${escapeHtml(c.name)}</option>`).join("")}
              </select>
            </div>
          </div>
        </div>
...
function tripStatusTone(status) {
  ...
  if (["completed", "approved", "confirmed"].includes(normalized)) return "success";
  if (["financial_review", "loaded", "unloading", "draft", "pending"].includes(normalized)) return "warning";
...
function filterTrips() {
  ...
  return PAGE_STATE.trips.filter((trip) => {
    ...
    const searchBlob = [trip.trip_no, trip.status, trip.route_name, trip.driver_name, trip.commodity_name, truckDisplay, trip.registration_no].join(" ").toLowerCase();
...
function sortTrips(rows) {
  ...
  sorted.sort((left, right) => {
    ...
    } else if (["quantity_mt", "client_gross_amount"].includes(key)) {
...
function tripKpis(rows) {
  return {
    ...
    grossAmount: rows.reduce((sum, trip) => sum + Number(trip.client_gross_amount || 0), 0)
  };
}
    ...
    <section class="trip-workspace">
      <section class="trip-kpi-grid">
        ${[
          ...
          ["Gross Billed", formatMoney(kpis.grossAmount)]
      ...
      <section class="card trip-filter-card">
        ...
        <div class="trip-toolbar-grid" role="search" aria-label="Trips filters">
          ...
          <div>
            ...
            <input id="tripNoFilterInput" type="text" value="${escapeHtml(tripFilters.tripNo)}" placeholder="TR2607001" />
          </div>
          <div>
          ...
          <table class="trip-rich-table">
            <thead>
              <tr>
                ...
                ${renderTripHeaderCell("Gross Amount", "client_gross_amount", "numeric")}
...
function renderTripRow(trip, index) {
  ...
  return `
    <tr class="trip-data-row ${index % 2 ? "is-striped" : ""}">
      <td><div class="trip-cell-stack"><strong>${escapeHtml(trip.trip_no || "-")}</strong><span class="muted">${escapeHtml(activeClient()?.name || "-")}</span></div></td>
      ...
      <td>${escapeHtml(`${formatMoney(trip.client_rate_per_mt).replace('.00', '')} / MT`)}</td>
      <td class="trip-numeric-cell">${escapeHtml(formatMoney(trip.client_gross_amount))}</td>
...
function renderTripDetailsModal() {
  ...
  const detailRows = [
    ...
    ["Rate", `${formatMoney(trip.client_rate_per_mt).replace('.00', '')} / MT`],
    ["Gross Amount", formatMoney(trip.client_gross_amount)],
      ...
      <div class="trip-modal-panel">
        <div class="stmt-actions" style="justify-content:space-between;margin-bottom:1rem;">
          <div>
            ...
            <p class="muted" style="margin:.25rem 0 0;">Review trip information for your account.</p>
          </div>
    ...
    <tr>
      <td colspan="11">
        <div class="trip-empty-state" role="status" aria-live="polite">
          ...
          <div class="muted">Try adjusting the filters or wait for new trips to be recorded.</div>
        </div>
      </td>
    </tr>
  `;
...
function renderGstInvoices() {
  return renderTable(
    ...
    PAGE_STATE.gstInvoices.map((i) => [i.invoice_no, formatDate(i.invoice_date), statusBadge(i.status), formatMoney(i.taxable_value), i.gst_percentage ?? "-", formatMoney(i.gst_amount), formatMoney(i.invoice_total), pdfButton("gst", i)]),
...
function renderReceipts() {
  return renderTable(
    ...
    PAGE_STATE.receipts.map((r) => [r.receipt_no, formatDate(r.receipt_date), statusBadge(r.status), formatMoney(r.amount_received), r.payment_mode || "-", r.reference_no || "-"]),
    "No receipts found."
  );
}
...
function renderDocuments() {
  return `
    <section class="card">
      <h3>Documents</h3>
      <p class="muted">Trips, bills, and GST invoices can each be downloaded as PDF from their respective sections. Additional document sharing is planned for a future release.</p>
    </section>
  `;
}
...
function render() {
  ...
  app.innerHTML = `
    <style>
      /* Scoped: same premium portal chrome as the transporter portal. */
          ...
          <div class="navbar-left">
            <div class="portal-topbar-brand">
              ...
              <div class="portal-topbar-copy">
                ...
                <strong class="portal-topbar-transporter">${escapeHtml(c?.name || "")}</strong>
              </div>
            </div>
          </div>
...
async function downloadTripPdf(id) {
  ...
  try {
    await exportPortalClientTripPdf({ trip: row, clientName: c?.name || "N/A" });
  } catch (error) {
    console.error(error);
