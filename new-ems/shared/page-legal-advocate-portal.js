...
function render() {
  const profile = state.context.profile || {};
  const shares = state.context.shares || [];
  const share = shares.find((row) => row.id === state.selectedShareId) || null;
...
function bind() {
  ...
  document.querySelectorAll("[data-status-shortcut]").forEach((button) => button.addEventListener("click", () => { state.status = button.dataset.statusShortcut; render(); }));
