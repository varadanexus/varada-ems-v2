...
function templatesViewV2(connections) {
  ...
  const rows = templates.map((template, index) => {
    ...
    return `<article class="wp-template-row" data-template-row data-template-index="${index}" data-template-status="${escapeHtml(String(template.status || "UNKNOWN").toUpperCase())}" role="button" tabindex="0" aria-label="View template ${escapeHtml(template.name)}"><div class="wp-template-icon">${escapeHtml((template.category || "T").charAt(0))}</div><div class="wp-template-copy"><div><strong>${escapeHtml(template.name)}</strong><span class="wp-template-status ${escapeHtml(String(template.status).toLowerCase())}">${escapeHtml(templateStatusLabel(template.status))}</span></div>${sid ? `<button class="wp-template-sid" type="button" data-copy-template-sid="${escapeHtml(sid)}" aria-label="Copy template SID ${escapeHtml(sid)}"><span>SID</span><code>${escapeHtml(sid)}</code><i>Copy</i></button>` : `<span class="wp-template-sid-pending">SID pending</span>`}<footer><span>${escapeHtml(template.category)}</span><span>${escapeHtml(template.language)}</span></footer></div><span class="wp-template-view-cue" aria-hidden="true"><span>Open</span> →</span></article>`;
  }).join("");
