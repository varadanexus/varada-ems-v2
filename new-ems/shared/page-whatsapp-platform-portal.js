...
function templatesViewV2(connections) {
  ...
  return `<section class="wp-route-page wp-templates-page"><div class="wp-route-heading wp-template-page-heading"><div><span class="wp-kicker">WhatsApp content studio</span><h1>Message templates</h1><p>Create, submit and manage reusable WhatsApp messages from one workspace.</p></div>${readyConnections.length ? `<div class="wp-template-create-actions"><button class="wp-secondary" id="wpCreateDocumentTemplateBtn" type="button"><span>▧</span> Create document</button><button class="wp-primary" id="wpCreateTemplateBtn" type="button"><span>＋</span> Create template</button></div>` : `<a class="wp-primary wp-button-link" href="${workspacePath("accounts")}">Connect account</a>`}</div>${workspaceTemplates.error ? `<div class="wp-verification-notice"><strong>Templates unavailable</strong><p>${escapeHtml(workspaceTemplates.error)}</p></div>` : ""}<section class="wp-template-stats" aria-label="Template overview"><article class="is-total"><div><span>Total templates</span><small>All saved content</small></div><strong>${templates.length}</strong></article><article class="is-draft"><div><span>Draft</span><small>Not submitted</small></div><strong>${count(["DRAFT"])}</strong></article><article class="is-review"><div><span>In review</span><small>Awaiting Meta</small></div><strong>${count(["PENDING","IN_REVIEW","IN_APPEAL"])}</strong></article><article class="is-rejected"><div><span>Rejected</span><small>Needs attention</small></div><strong>${count(["REJECTED"])}</strong></article><article class="is-approved"><div><span>Approved</span><small>Ready to send</small></div><strong>${count(["APPROVED"])}</strong></article></section><nav class="wp-template-mode-tabs" aria-label="Template collections"><button class="active" type="button" data-template-panel-tab="owned"><span class="wp-template-tab-icon">◇</span><span>My templates<small>Create and manage</small></span></button><button type="button" data-template-panel-tab="library"><span class="wp-template-tab-icon">▦</span><span>Meta library<small>${libraryTemplates.length} pre-approved designs</small></span></button></nav><section class="wp-card wp-template-library" data-template-panel="owned"><header><div><span class="wp-card-eyebrow">Template inventory</span><h2>Your WhatsApp templates</h2><p>Search, filter and inspect every template connected to this workspace.</p></div><div class="wp-template-tools">${selector}<label class="wp-inbox-search"><span>⌕</span><input type="search" placeholder="Search by name or message" aria-label="Search templates" data-template-search /></label><button class="wp-secondary wp-icon-button" id="wpRefreshTemplatesBtn" type="button" aria-label="Refresh templates"><span>↻</span> Refresh</button></div></header><nav class="wp-template-status-filters" aria-label="Template status"><button class="active" type="button" data-template-status-filter="ALL">All <span>${templates.length}</span></button><button type="button" data-template-status-filter="DRAFT">Draft <span>${count(["DRAFT"])}</span></button><button type="button" data-template-status-filter="IN_REVIEW">In review <span>${count(["PENDING","IN_REVIEW","IN_APPEAL"])}</span></button><button type="button" data-template-status-filter="REJECTED">Rejected <span>${count(["REJECTED"])}</span></button><button type="button" data-template-status-filter="APPROVED">Approved <span>${count(["APPROVED"])}</span></button></nav><div class="wp-template-list">${rows || `<div class="wp-inbox-empty"><span>✦</span><strong>No templates yet</strong><p>Create a custom template or choose one from Meta's library.</p></div>`}</div></section><section class="wp-card wp-meta-library" data-template-panel="library" hidden><header><div><span class="wp-card-eyebrow">Pre-approved by Meta</span><h2>Template library</h2><p>Start from ready-made Utility and Authentication structures.</p></div><div class="wp-template-tools"><select id="wpLibraryCategory" aria-label="Library category"><option value="UTILITY" ${workspaceTemplateLibrary.category === "UTILITY" ? "selected" : ""}>Utility</option><option value="AUTHENTICATION" ${workspaceTemplateLibrary.category === "AUTHENTICATION" ? "selected" : ""}>Authentication</option></select><select id="wpLibraryLanguage" aria-label="Library language"><option value="en_US">English (US)</option><option value="en_GB">English (UK)</option><option value="hi">Hindi</option><option value="te">Telugu</option></select><label class="wp-inbox-search"><span>⌕</span><input type="search" placeholder="Search library" data-library-search /></label></div></header>${workspaceTemplateLibrary.error ? `<div class="wp-verification-notice"><strong>Library unavailable</strong><p>${escapeHtml(workspaceTemplateLibrary.error)}</p></div>` : ""}<div class="wp-meta-library-grid">${libraryTemplates.map(libraryTemplateCard).join("") || `<div class="wp-inbox-empty"><span>⌕</span><strong>No library templates found</strong><p>Try another category or language.</p></div>`}</div></section>${templateBuilderDialog(readyConnections, selectedId)}${libraryCloneDialog(readyConnections, selectedId)}</section>`;
}
