...
function openBillingDocument(documentRecord, kind = "invoice") {
...
function billingAccessRequiredView() {
  ...
  return `<section class="wp-route-page wp-billing-locked"><div class="wp-billing-lock-card"><span class="wp-billing-lock-icon" aria-hidden="true">₹</span><span class="wp-card-eyebrow">${escapeHtml(stateLabel)}</span><h1>Billing access is required</h1><p>${escapeHtml(entitlement.reason || "A valid authorized trial or active paid subscription is required to use this workspace.")}</p>${canManage ? `<div class="wp-billing-lock-actions"><a class="wp-primary wp-button-link" href="${workspacePath("billing")}">Review billing &amp; activate</a><a class="wp-secondary wp-button-link" href="/contact.html">Contact billing support</a></div><small>Login, billing recovery and secure checkout remain available. Product APIs stay locked until Razorpay confirms authorization or payment.</small>` : `<div class="wp-policy-note"><strong>Ask your workspace owner to restore access</strong><p>Only the owner or a billing administrator can authorize or renew the subscription.</p></div>`}</div></section>`;
}
...
async function renderDashboard() {
  ...
  if (view === "billing") {
    app.querySelectorAll("[data-billing-document]").forEach((button) => button.addEventListener("click", () => {
      try {
        ...
        openBillingDocument(record, kind);
      } catch (error) { showToast(error?.message || "Billing document could not be opened.", "error"); }
    }));
  ...
  if (view === "checkout") {
    ...
    quoteForm?.addEventListener("submit", async (event) => {
      ...
      const addons = [...form.querySelectorAll("[data-checkout-addon]:checked")].map((input) => ({
        ...
        quantity: Number(form.querySelector(`[data-checkout-addon-quantity="${CSS.escape(input.dataset.checkoutAddon)}"]`)?.value || 0),
      }));
      const original = button.textContent;
      try {
