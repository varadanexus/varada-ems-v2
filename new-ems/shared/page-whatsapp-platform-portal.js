...
async function openBillingDocument(documentRecord, kind = "invoice") {
  ...
  dialog.innerHTML = `<section><header><div><span>FINANCIAL DOCUMENT</span><h2>${escapeHtml(result.documentNumber || "Billing document")}</h2><p>Archived securely in ${escapeHtml(result.driveFolderPath || "the dedicated Google Drive folder")}</p></div><div><a class="wp-secondary wp-button-link" href="${pdfUrl}" download="${escapeHtml(result.fileName || "billing-document.pdf")}">Download PDF</a><button class="wp-primary" type="button" data-billing-pdf-print>Print</button><button class="wp-billing-document-close" type="button" aria-label="Close PDF preview">×</button></div></header><main><iframe title="${escapeHtml(result.documentNumber || "Billing document")} PDF preview" src="${pdfUrl}"></iframe></main><footer><span><b>Drive archived</b> ${escapeHtml(formatProfileDate(result.archivedAt))}</span><span>Centralized EMS number · Razorpay references preserved</span></footer></section>`;
...
function billingView() {
  ...
  const invoices = (workspaceBilling?.invoices || []).map((invoice) => `<tr><td><strong>${escapeHtml(invoice.invoice_number)}</strong><small>${escapeHtml(formatProfileDate(invoice.invoice_date))}${invoice.document_environment === "test" ? " · Test document" : ""}${invoice.pdf_archived_at ? " · Drive archived" : " · Archive pending"}</small></td><td><span class="wp-billing-payment-status ${escapeHtml(invoice.status)}">${escapeHtml(invoice.status)}</span></td><td><small>Razorpay invoice</small>${escapeHtml(invoice.provider_invoice_id)}<small>Transaction</small>${escapeHtml(invoice.provider_payment_id)}</td><td>${escapeHtml(billingMoney(Number(invoice.total_paise || 0) / 100, invoice.currency))}</td><td><button class="wp-secondary" type="button" data-billing-document="invoice:${escapeHtml(invoice.id)}">View PDF</button></td></tr>`).join("");
  const creditNotes = (workspaceBilling?.creditNotes || []).map((note) => `<tr><td><strong>${escapeHtml(note.credit_note_number)}</strong><small>${escapeHtml(formatProfileDate(note.credit_note_date))}${note.document_environment === "test" ? " · Test document" : ""}${note.pdf_archived_at ? " · Drive archived" : " · Archive pending"}</small></td><td>${escapeHtml(note.reason || "Razorpay refund")}</td><td>${escapeHtml(note.provider_refund_id)}</td><td>${escapeHtml(billingMoney(Number(note.total_paise || 0) / 100, note.currency))}</td><td><button class="wp-secondary" type="button" data-billing-document="credit_note:${escapeHtml(note.id)}">View PDF</button></td></tr>`).join("");
