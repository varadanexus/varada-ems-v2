...
let workspaceBilling = { configured: false, mode: "test", packages: [], subscription: null, payments: [], customer: null, error: "" };
...
function analyticsView() {
  ...
  const readyCampaigns = campaigns.filter((campaign) => campaign.status === "approved" && new Date(campaign.scheduledAt || 0).getTime() > Date.now());
  const campaignTotal = Math.max(campaigns.length, 1);
  const statusBars = ["draft", "review", "approved", "rejected", "paused"].map((status) => {
  ...
  return `<section class="wp-route-page wp-analytics-page">
    ...
    <section class="wp-analytics-grid">
      ...
      <article class="wp-card wp-analytics-panel wp-analytics-locked"><span class="wp-card-eyebrow">Production performance</span><div class="wp-analytics-lock-icon" aria-hidden="true">◇</div><h2>Messaging outcomes activate after launch</h2><p>Delivery, read, reply, failure and campaign-conversion metrics will appear only after production sending is enabled and real events are received.</p><ul><li>Delivery and failure rate</li><li>Read and response rate</li><li>Template and campaign comparison</li></ul><small>No simulated performance data is shown.</small></article>
    </section>
  </section>`;
}
...
function billingView() {
  ...
  const heading = `<div class="wp-route-heading"><div><span class="wp-kicker">Subscription management</span><h1>Billing &amp; usage</h1><p>Choose a package, pay securely with Razorpay and manage the subscription for this workspace. Displayed subscription prices include payment-gateway fees and GST on those fees.</p></div><div class="wp-billing-heading-actions"><span class="wp-billing-mode ${workspaceBilling?.mode === "live" ? "is-live" : ""}">${escapeHtml(workspaceBilling?.mode === "live" ? "Live payments" : "Test payments")}</span><a class="wp-secondary wp-button-link" href="/contact.html?subject=WhatsApp%20billing%20support">Billing support</a></div></div>`;
...
async function renderDashboard() {
  ...
  if (!agentWorkspace) {
    ...
    if (view === "billing") {
      ...
      catch (error) { workspaceBilling = { configured: false, mode: "test", packages: [], subscription: null, payments: [], customer: null, error: error?.message || "Billing could not be loaded." }; }
    } else {
      workspaceBilling = { configured: false, mode: "test", packages: [], subscription: null, payments: [], customer: null, error: "" };
    }
  } else {
    metaOnboardingStatus = {
      configured: Boolean(runtime.metaAppId && runtime.embeddedSignupConfigId),
