...
export function mountWalletAdmin(host,request,tenants,readiness=null) {
  host.classList.add('wa-billing-controls');
  let selected=null,busy=false,loadRevision=0;
  const readinessCopy=readiness ? `<div class="wa-billing-readiness"><header><div><span class="wa-admin-kicker">Payment infrastructure</span><h4>Billing readiness</h4></div><span class="wa-price-audit-badge">${esc(readiness.billingMode)} mode</span></header><div class="wa-billing-readiness-grid">${[['PAYG gate',readiness.gateEnabled,'Enabled','Disabled'],['Gateway keys',readiness.razorpayKeyConfigured,'Configured','Missing'],['Webhook secret',readiness.webhookSecretConfigured,'Configured','Missing'],['Public webhook',readiness.publicWebhookConfigured,'Configured','Missing'],['Checkout gate',readiness.walletCheckoutEnabled,'Enabled','Disabled']].map(([label,ok,yes,no])=>`<div class="${ok?'is-ready':'is-pending'}"><span>${label}</span><strong><i aria-hidden="true"></i>${ok?yes:no}</strong></div>`).join('')}</div><p>Infrastructure status only—not confirmation of customer payment activation. Every wallet prerequisite must still be verified.</p></div>` : '';
