...
function metrics() {
  ...
  return `<section class="wa-admin-stats">
    ...
    <article class="wa-admin-stat"><span>Authentication</span><strong>Local</strong><small>Isolated from EMS Auth</small></article>
  </section>`;
}
...
function overview() {
  ...
  return `${metrics()}<section class="wa-admin-grid">
    ...
    <article class="wa-admin-card"><h3>Product readiness</h3><p>Operational controls for launching the customer-facing application.</p><div class="wa-admin-checklist">
      <div class="wa-admin-check"><span class="wa-admin-check-icon">✓</span><div><strong>Customer authentication</strong><small>Local password hashing and revocable sessions</small></div>${status("ready")}</div>
...
function security() {
  ...
  return `<section class="wa-admin-grid"><article class="wa-admin-card"><h3>Customer identity boundary</h3><p>The sellable product uses its own local authentication system and does not use Supabase Auth customer accounts.</p><div class="wa-admin-list"><div class="wa-admin-row"><strong>Password storage</strong><span>One-way crypt hashing</span></div><div class="wa-admin-row"><strong>Session storage</strong><span>Hashed server-side tokens</span></div><div class="wa-admin-row"><strong>Session lifetime</strong><span>12 hours</span></div><div class="wa-admin-row"><strong>Browser persistence</strong><span>Current tab only</span></div><div class="wa-admin-row"><strong>Meta/provider secrets</strong><span>Server-side only</span></div></div></article><article class="wa-admin-card"><h3>Security activity</h3><p>Aggregate authentication signals; raw credentials and session tokens are never returned.</p><div class="wa-admin-list"><div class="wa-admin-row"><strong>Login attempts · 24h</strong><span>${Number(auth.loginAttempts24h || 0)}</span></div><div class="wa-admin-row"><strong>Signup attempts · 24h</strong><span>${Number(auth.signupAttempts24h || 0)}</span></div><div class="wa-admin-row"><strong>Active sessions</strong><span>${Number(state.snapshot?.totals?.activeSessions || 0)}</span></div><div class="wa-admin-row"><strong>Expired/revoked sessions</strong><span>${Number(auth.inactiveSessions || 0)}</span></div></div></article></section>`;
}
