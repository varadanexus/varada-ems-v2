-- Sprint 20: Multiple agents per truck with configurable commission split.
--
-- The truck-agent mapping already allows several agents per truck (one row per
-- truck+agent, date-windowed). This migration makes the money model explicit:
-- each mapping row keeps its own commission_type + commission_value (independent
-- rate) AND gains a commission_share_percentage that scales that agent's slice.
--
--   * Independent model : give each agent their own rate, leave share = 100.
--   * Pure split model  : give agents the same rate, set shares that sum to 100.
--   * Mixed             : any combination.
--
-- Existing single-agent rows auto-convert to share = 100 (default), so behaviour
-- is byte-for-byte identical until a second agent is added. Every commission
-- calculation funnels through the three functions redefined below, so applying
-- the share factor here is sufficient across admin + agent-portal surfaces.

-- 1. Share column + range guard ---------------------------------------------
alter table public.transport_truck_agent_commission_mapping
  add column if not exists commission_share_percentage numeric(6,3) not null default 100;

update public.transport_truck_agent_commission_mapping
  set commission_share_percentage = 100
  where commission_share_percentage is null;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'chk_truck_agent_commission_share_range'
  ) then
    alter table public.transport_truck_agent_commission_mapping
      add constraint chk_truck_agent_commission_share_range
      check (commission_share_percentage >= 0 and commission_share_percentage <= 100);
  end if;
end;
$$;

-- 2. Admin/earned commission per agent (drives withdrawal balances) ----------
...
-- 4. Agent portal per-trip commission list ----------------------------------
