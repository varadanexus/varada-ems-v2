...
do $$
begin
  if to_regclass('public.transport_clients') is not null then
    ...
    raise notice 'Skipping Sprint 6A seed repair block: public.transport_clients does not exist';
  end if;
end $$;

do $$
begin
  if to_regclass('public.transport_transporters') is not null then
    ...
    raise notice 'Skipping Sprint 6A seed repair block: public.transport_transporters does not exist';
  end if;
end $$;

do $$
begin
  if to_regclass('public.transport_agents') is not null then
    ...
    raise notice 'Skipping Sprint 6A seed repair block: public.transport_agents does not exist';
  end if;
end $$;

do $$
begin
  if to_regclass('public.transport_commodities') is not null then
    ...
    raise notice 'Skipping Sprint 6A seed repair block: public.transport_commodities does not exist';
  end if;
end $$;

do $$
begin
  if to_regclass('public.transport_client_mapping') is not null
     and to_regclass('public.transport_clients') is not null
     and to_regclass('public.transport_route_master') is not null
     and to_regclass('public.transport_commodities') is not null then
    ...
    raise notice 'Skipping Sprint 6A seed repair block: client mapping dependencies do not exist';
  end if;
end $$;

do $$
begin
  if to_regclass('public.transport_transporter_mapping') is not null
     and to_regclass('public.transport_transporters') is not null
     and to_regclass('public.transport_trucks') is not null
     and to_regclass('public.transport_route_master') is not null
     and to_regclass('public.transport_commodities') is not null then
    with d as (select id from public.divisions where code = 'TRANSPORT' limit 1),
    ...
    raise notice 'Skipping Sprint 6A seed repair block: transporter mapping dependencies do not exist';
  end if;
end $$;

do $$
begin
  if to_regclass('public.transport_rate_master') is not null
     and to_regclass('public.transport_clients') is not null
     and to_regclass('public.transport_transporters') is not null
     and to_regclass('public.transport_route_master') is not null
     and to_regclass('public.transport_commodities') is not null
     and to_regclass('public.transport_truck_owners') is not null
     and to_regclass('public.transport_trucks') is not null then
    with d as (select id from public.divisions where code = 'TRANSPORT' limit 1),
    ...
    raise notice 'Skipping Sprint 6A seed repair block: rate master dependencies do not exist';
  end if;
end $$;