-- Sprint 8B.1: Fix GST transporter cost to use direct selected trip cost, not statement-derived values
...
begin
  ...
  select round(coalesce(sum(eligible.client_net_receivable), 0)::numeric, 2),
         round(coalesce(sum(eligible.transporter_gross_payable), 0)::numeric, 2)
