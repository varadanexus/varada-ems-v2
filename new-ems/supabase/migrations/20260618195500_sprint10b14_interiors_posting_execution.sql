-- Sprint 10B.14: Interiors Central Accounts posting execution
-- Authorized posting only. No transport changes.

create extension if not exists pgcrypto;

with seed_accounts(code, name, account_class, account_group, is_control_account) as (
  values
    ('1310', 'Interior Receivables', 'asset', 'receivables', true),
    ('4110', 'Interior Revenue', 'income', 'operating_revenue', false)
)
...
select code, name, account_class, account_group, 4, true, is_control_account, true
from seed_accounts s
where not exists (select 1 from public.coa_accounts c where c.code = s.code);

drop function if exists public.execute_central_accounts_interiors_posting(uuid);

create or replace function public.execute_central_accounts_interiors_posting(
...
begin
  ...
  if not public.can_manage_central_accounts() then
...
exception when others then
  ...
  perform public.log_central_accounts_audit_event(
    ...
    coalesce(v_fd.document_family, 'INTERIOR_BILL'),
    coalesce(v_fd.source_document_id::text, p_financial_document_id::text),
