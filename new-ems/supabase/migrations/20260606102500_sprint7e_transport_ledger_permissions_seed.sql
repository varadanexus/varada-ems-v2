-- Sprint 7E: transport ledger permissions seed

with seed_modules(module_code) as (
  values ('transport-ledger')
),
...
select m.module_code,
       a.action_code,
       'Transport Ledger ' || initcap(a.action_code),
       true
from seed_modules m
...
from public.roles r
join public.permissions p on p.module_code = 'transport-ledger' and p.action_code in ('view','edit')
