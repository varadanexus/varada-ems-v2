-- Include verified PAYG standalone WhatsApp-number periods in the existing
-- number-capacity reconciler. Legacy package/add-on capacity remains intact;
-- only captured, live-mode periods that are active at the reconciliation time
-- contribute additional production slots. Test-number connections remain
-- excluded by the original reconciler.
...
declare
  ...
  v_legacy_additional_limit integer := 0;
  v_payg_additional_limit integer := 0;
...
begin
  ...
  into v_legacy_additional_limit
  ...
  -- Capacity is granted only from a verified, captured PAYG period. The
  -- production-number reconciler intentionally consumes live-mode evidence;
  -- developer test numbers never consume these slots.
  select least(coalesce(sum(p.quantity), 0), 2147483647)::integer
    into v_payg_additional_limit
  from public.whatsapp_platform_payg_capacity_periods p
  where p.tenant_id = p_tenant_id
    and p.mode = 'live'
    and p.addon_code = 'extra_whatsapp_number'
    and p.paid_from <= now()
    and p.paid_until > now()
    and not exists (
      select 1
      from public.whatsapp_platform_payg_capacity_corrections c
      where c.period_id = p.id
        and c.effective_at <= now()
    );

  v_additional_limit := least(v_legacy_additional_limit + v_payg_additional_limit, 2147483647);
  ...
  return jsonb_build_object(
    ...
    'legacyAdditionalLimit', v_legacy_additional_limit,
    'paygAdditionalLimit', v_payg_additional_limit,
