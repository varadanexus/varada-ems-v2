-- Return a customer case to an editable state whenever protected evidence is
-- rejected or needs replacement.
...
begin
  ...
  set review_status=p_decision, review_notes=nullif(trim(coalesce(p_notes,'')),''),
  ...
  set status=case
        when p_decision in ('changes_requested','rejected') then 'changes_requested'
        when p_decision='in_review' and status in ('submitted','verified') then 'in_review'
        else status end,
      review_notes=case when p_decision in ('changes_requested','rejected')
        then nullif(trim(coalesce(p_notes,'')),'') else review_notes end,
      updated_at=now()
  ...
  set verification_status=case
        when p_decision in ('changes_requested','rejected') then 'changes_requested'
        when p_decision='in_review' and t.verification_status in ('submitted','verified') then 'in_review'
        else t.verification_status end,
...
-- Repair document decisions recorded before the transition above existed.
update public.whatsapp_platform_business_verifications v
set status='changes_requested',
    review_notes=coalesce(nullif(v.review_notes,''), (
      select d.review_notes from public.whatsapp_platform_verification_documents d
      where d.verification_id=v.id and d.status='active'
        and d.review_status in ('changes_requested','rejected')
      order by d.reviewed_at desc nulls last limit 1
    )),
    updated_at=now()
where v.status in ('submitted','in_review','verified')
  and exists (select 1 from public.whatsapp_platform_verification_documents d
    where d.verification_id=v.id and d.status='active'
      and d.review_status in ('changes_requested','rejected'));

update public.whatsapp_platform_tenants t
set verification_status='changes_requested', verified_at=null, updated_at=now()
where t.verification_status in ('submitted','in_review','verified')
  and exists (select 1 from public.whatsapp_platform_verification_documents d
