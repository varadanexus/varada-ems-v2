-- Administrative visibility and fresh-acceptance enforcement for internal EMS users.
-- The underlying evidence tables remain inaccessible directly; these narrow RPCs
-- require the existing Users view/edit permissions and expose one requested user.
...
begin
  ...
  select version, title, effective_at
  into v_terms
  ...
  select id, requested_at, reason, requested_by
  into v_request
