...
function b64(bytes:Uint8Array){ let binary=""; bytes.forEach(v=>binary+=String.fromCharCode(v)); return btoa(binary).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,""); }
function unb64(value:string){ const raw=value.replace(/-/g,"+").replace(/_/g,"/").padEnd(Math.ceil(value.length/4)*4,"="); return Uint8Array.from(atob(raw),c=>c.charCodeAt(0)); }
async function secretKey(){ const material=env("WHATSAPP_PLATFORM_TOKEN_ENCRYPTION_KEY"); if(material.length<32) throw new Error("Integration Lab encryption is not configured."); const digest=await crypto.subtle.digest("SHA-256",encoder.encode(`developer-integrations:${material}`)); return crypto.subtle.importKey("raw",digest,"AES-GCM",false,["encrypt","decrypt"]); }
async function encrypt(value:string){ const iv=crypto.getRandomValues(new Uint8Array(12)); const encrypted=await crypto.subtle.encrypt({name:"AES-GCM",iv},await secretKey(),encoder.encode(value)); return `v1.${b64(iv)}.${b64(new Uint8Array(encrypted))}`; }
async function decrypt(value:string){ const [version,iv,data]=String(value||"").split("."); if(version!=="v1") throw new Error("Integration Lab credential is invalid."); const clear=await crypto.subtle.decrypt({name:"AES-GCM",iv:unb64(iv)},await secretKey(),unb64(data)); return new TextDecoder().decode(clear); }
...
async function provision(req:Request,admin:any,customer:any,body:any){
  ...
  if(body.connectionId)connectionQuery=connectionQuery.eq("id",String(body.connectionId));
  const {data:connections,error:connectionError}=await connectionQuery.limit(1); const connection=connections?.[0]; if(connectionError||!connection)throw new Error("Connect a WhatsApp business number before configuring Integration Lab.");
  const prefix=randomHex(4), apiToken=`vn_live_${prefix}_${randomHex(24)}`, webhookSecret=`whsec_${randomHex(32)}`, receiverToken=randomHex(32);
  const {data:key,error:keyError}=await admin.from("whatsapp_platform_api_keys").insert({tenant_id:customer.tenant_id,name:"EMS Integration Lab",key_prefix:`vn_live_${prefix}`,token_hash:await sha256(apiToken),scopes:["contacts:read","contacts:write","messages:write"],status:"active",created_by:customer.user_id}).select("id").single(); if(keyError)throw keyError;
  const receiverUrl=`${env("SUPABASE_URL").replace(/\/+$/,'')}/functions/v1/whatsapp-platform-integration-lab?receiver=${receiverToken}`;
  const {data:endpoint,error:endpointError}=await admin.from("whatsapp_platform_webhook_endpoints").insert({tenant_id:customer.tenant_id,connection_id:connection.id,software_name:"EMS Integration Lab",name:"Managed integration test receiver",endpoint_url:receiverUrl,max_attempts:4,fallback_max_attempts:0,timeout_ms:10000,signing_secret_ciphertext:await encrypt(webhookSecret),events:LAB_EVENTS,status:"active",created_by:customer.user_id}).select("id").single();
  if(endpointError){ await admin.from("whatsapp_platform_api_keys").delete().eq("id",key.id); throw endpointError; }
  const {error:labError}=await admin.from("whatsapp_platform_integration_labs").insert({tenant_id:customer.tenant_id,connection_id:connection.id,api_key_id:key.id,api_token_ciphertext:await encrypt(apiToken),webhook_endpoint_id:endpoint.id,webhook_secret_ciphertext:await encrypt(webhookSecret),receiver_token_hash:await sha256(receiverToken),created_by:customer.user_id}); if(labError)throw labError;
  return summary(admin,customer);
}
async function receiver(req:Request,admin:any,token:string){ const raw=await req.text(); const {data:lab}=await admin.from("whatsapp_platform_integration_labs").select("*").eq("receiver_token_hash",await sha256(token)).eq("status","active").maybeSingle(); if(!lab)return json(req,{error:"Unknown receiver"},404); const supplied=String(req.headers.get("x-varada-signature")||"").replace(/^sha256=/,""); const valid=equal(supplied,await hmac(await decrypt(lab.webhook_secret_ciphertext),raw)); if(!valid)return json(req,{error:"Invalid signature"},401); const payload=JSON.parse(raw||"{}"); await admin.from("whatsapp_platform_integration_lab_events").upsert({tenant_id:lab.tenant_id,lab_id:lab.id,event_id:String(req.headers.get("x-varada-event-id")||payload.id||crypto.randomUUID()),event_type:String(req.headers.get("x-varada-event")||payload.type||"unknown"),signature_valid:true,payload},{onConflict:"lab_id,event_id",ignoreDuplicates:true}); return json(req,{received:true}); }
async function runCheck(name:string,fn:()=>Promise<any>){ const started=Date.now(); try{ const result=await fn(); return {name,status:"passed",durationMs:Date.now()-started,...result}; }catch(error){ return {name,status:"failed",durationMs:Date.now()-started,message:error instanceof Error?error.message:String(error)}; } }
async function runSafe(req:Request,admin:any,customer:any,body:any){ const lab=await labForTenant(admin,customer.tenant_id); if(!lab)throw new Error("Configure Integration Lab first."); const started=Date.now(); const {data:run,error}=await admin.from("whatsapp_platform_integration_lab_runs").insert({tenant_id:customer.tenant_id,lab_id:lab.id,suite:"safe",status:"running",started_by:customer.user_id}).select("id").single(); if(error)throw error; const base=`${env("SUPABASE_URL").replace(/\/+$/,'')}/functions/v1/whatsapp-platform-api`; const token=await decrypt(lab.api_token_ciphertext);
  const request=async(path:string,auth=true)=>{ const response=await fetch(`${base}${path}`,{headers:auth?{Authorization:`Bearer ${token}`}:{}}); const data=await response.json().catch(()=>({})); return {response,data}; };
  const results=[];
  results.push(await runCheck("Rejects missing API credentials",async()=>{const {response}=await request("/v1/contacts",false); if(response.status!==401)throw new Error(`Expected 401, received ${response.status}`); return {httpStatus:response.status};}));
  results.push(await runCheck("Lists connected WhatsApp numbers",async()=>{const {response,data}=await request("/v1/numbers"); if(!response.ok)throw new Error(data?.error?.message||`HTTP ${response.status}`); return {httpStatus:response.status,count:data.numbers?.length||0,requestId:data.requestId};}));
