...
### Immutable mandate provider-order binding

- Added local migration `20260916211000_whatsapp_wallet_mandate_order_binding.sql` with a protected immutable registration-to-customer/order projection and server-only binding RPC. It requires a pending tenant/mode registration and server-fetched customer notes proving tenant/mode/purpose, plus exact zero-value INR order receipt and registration metadata. A nonzero recharge or unrelated customer/order cannot bind.
- Exact replay retains original provider identities; conflicting replay rejects. Full provider objects are not stored. Recovery evidence can bind/replay after settings change during an external operation; activation verification must still reject stale settings. Binding never releases the pending slot or activates/credits/debits anything.
...
### Authenticated Test-only mandate service

- Added `whatsapp-wallet-mandate-service.ts` factory connecting begin, customer/order authorization, signed callback verification/evidence persistence and explicit registration closure. It defaults disabled and rejects Live mode. Current pending-slot and original consent actor are loaded/checked before authorization/verification; all reads are tenant/mode scoped.
- Provider IDs come only from protected order/customer mappings, not browser request fields. Stored database integer strings are normalized with safe-integer validation. Signed callback is checked before provider verification, and the browser result omits token/customer-sensitive evidence and states `autoTopupActive:false`.
- Dedicated service fixtures pass for stored identity use despite injected browser IDs, Test-only/disabled/role guards, missing current slot, different consent actor, invalid HMAC and private evidence projection. This factory is not yet imported/dispatched by the billing endpoint or customer UI.
- Next wire gated Test-only endpoint actions using encrypted Test credentials and authoritative profile lookup; review/apply the five mandate migrations, then build consent UI and perform actual provider Test registration/verification. Auto-top-up activation/worker gross caps, retries and end-to-end debit tests still remain. Do not switch production runtime or infer financial confirmation.

### Local billing endpoint wiring and Git-lock recovery

- Canonical billing endpoint now imports the mandate service/helpers and dispatches rate-limited `wallet_mandate_test_begin/authorise/verify/close` actions. These require `WHATSAPP_MANDATE_TEST_ENABLED=true` and exact `WHATSAPP_MANDATE_TEST_TENANT_ID`, and use only dedicated `RAZORPAY_TEST_KEY_ID/SECRET` or an existing explicitly Test-prefixed legacy environment credential pair. Live credentials are rejected; normal production mode/settings are not changed or loaded for these actions.
- Provider profile identity is read server-side from the authenticated active tenant/user's display_name/email/contact_phone. No arbitrary browser profile/provider identity is adopted. Endpoint remains LOCAL ONLY and its feature flag has not been enabled remotely.
- Full suite passed with 469 canonical migrations; billing TypeScript parse passed. Added factory fixtures cover production-mode preservation, separate Test pair, missing Test secret, Live credential rejection and tenant/feature gates.
- The unchanged zero-byte `.git/index.lock` was revalidated against its exact timestamp/path with no running Git processes, then removed using a single literal-path native operation. No index contents, repository data, user changes or worktree files were deleted/reset/cleaned.
- Next review/apply only the five pending mandate migrations and deploy the disabled endpoint, verify remote status, then enable only scoped Test access after credential/method checks and implement consent UI. Real Test mandate/debit verification and automatic recharge limits/worker remain unfinished; Live activation still requires financial decisions and immediate confirmation.

### Remote disabled mandate-backend deployment

- `supabase db push --linked --dry-run` listed exactly the five mandate migrations `20260916210000` through `20260916214000`, with no unrelated pending migration. All five were then applied successfully using the linked canonical `new-ems` project.
- Canonical billing function deployment succeeded and independent remote listing verifies `whatsapp-platform-billing` ACTIVE, version 81. The uploaded bundle includes the mandate service/helpers. No financial activation/action was submitted.
- Fresh secret-name-only inventory confirms absence of `WHATSAPP_MANDATE_TEST_ENABLED` and `WHATSAPP_MANDATE_TEST_TENANT_ID`, so the deployed Test actions remain disabled. Dedicated `RAZORPAY_TEST_KEY_ID/SECRET` names are also absent; the separate guarded legacy environment Test-pair fallback remains unverified. Do not assume missing Test credentials merely from missing dedicated names, and do not overwrite the saved Live provider settings to test.
- Next implement scoped Test credential/setup management and consent UI, verify Test provider method availability, enable only the intended Test tenant after readiness checks, and complete actual Test registration/callback/evidence flow. Automatic recharge worker/gross-cap enforcement and real Test subsequent-debit verification still remain. Live charging remains unactivated pending prior paid-period/FX/charge-policy decisions and final confirmation.
...
### Live-only Razorpay credential control and proposed USD wallet model

- On 16 September 2026 the user clarified that the active payment configuration must be Live Mode only, with no Test/Live ambiguity. The uncommitted proposal for a second database-managed Test credential set and its migration was removed before deployment; no Test setting, secret, mode switch or migration was applied.
- EMS Razorpay Settings now labels the production configuration explicitly as Live Mode, accepts only an `rzp_live_...` Key ID, and identifies the branded webhook as the Live webhook. The server-side `set_razorpay` action independently rejects non-Live Key IDs and always records `provider_mode=live`. This does not expose, rotate or otherwise modify the already encrypted credentials until an authorized operator submits the form.
- Focused admin-surface tests, TypeScript parsing and the complete PAYG suite passed after the Live-only change. At this checkpoint the change is local and still needs a narrow commit, push and deployment of `whatsapp-platform-admin-secrets` plus the EMS Pages asset; do not deploy the billing endpoint for this UI-only safety change.
- The user proposed a different accounting model: display and deduct all service wallet balances in USD, while collecting INR from confirmed Indian billing entities and USD from confirmed foreign billing entities. Current implementation instead keeps the wallet ledger and visible balance in the selected native currency and converts the USD message price per usage event.
- Recommended redesign, pending explicit financial approval: ledger/balance always USD micros; separate confirmed billing country and payment currency; INR checkout for India and USD checkout elsewhere; immutable recharge evidence preserving payment currency/amount, USD credit, accepted FX quote, tax/gateway lines and provider IDs. Profile country may prefill the choice but must not silently determine tax/payment treatment; require one-time confirmation and lock it after first captured payment.
- No wallet schema, existing balance, quote, coupon, invoice, auto-top-up or financial record was changed for this proposal. The Test wallet has zero current balance and no Live wallet is active, so this is the safest point to redesign if the user explicitly approves it. Historical INR Test payment/recharge evidence must remain immutable.
