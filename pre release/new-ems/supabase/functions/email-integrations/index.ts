...
function zeptoConfigStatus() {
  return {
    zepto: {
      apiBaseUrl: Boolean(
        trimText(env("ZEPTO_API_BASE_URL", "https://api.zeptomail.in")),
      ),
...
function providerHealth() {
  const baseUrl = trimText(
    env("ZEPTO_API_BASE_URL", "https://api.zeptomail.in"),
  );
  ...
  if (!fromEmail || !fromEmail.includes("@"))
    errors.push("From email is missing or invalid");
  ...
  return {
    zeptoApi: errors.length
      ...
      : healthMessage(
          true,
          `Ready to send through ${baseUrl.replace(/\/+$/, "")}/v1.1/email`,
        ),
  };
}
...
async function requireAdminCaller(req: Request, admin: any) {
  ...
  const roleCodes = (roles || [])
    ...
    .filter(Boolean);
  if (
    !roleCodes.some((code: string) => ["super_admin", "admin"].includes(code))
  ) {
  ...
  return [
    {
      ...
      name:
        trimText(env("ZEPTO_REPLY_TO_NAME")) ||
        trimText(env("ZEPTO_FROM_NAME")) ||
        "Varada Nexus",
    },
  ];
}
...
function buildEmailPayload(payload: Record<string, any>) {
  ...
  const fromOverride =
    payload.from && payload.from.address
      ? {
          address: normalizeEmail(payload.from.address),
          name:
            trimText(payload.from.name) || normalizeEmail(payload.from.address),
        }
      : {
          address: normalizeEmail(env("ZEPTO_FROM_EMAIL")),
          name: trimText(env("ZEPTO_FROM_NAME")) || "Varada Nexus",
        };
  ...
  if (cc.length)
    body.cc = cc.map((item) => buildRecipient(item.address, item.name));
  if (bcc.length)
    body.bcc = bcc.map((item) => buildRecipient(item.address, item.name));
...
const SOURCE_SENDER_RULES: Array<{ pattern: RegExp; senderKey: string }> = [
  ...
  {
    pattern: /digital[-_ ]?services|digital[-_ ]?marketing|marketing|vendor/,
    senderKey: "digitalmarketing",
  },
  {
    pattern: /\bhr\b|human[-_ ]?resources|employee|payroll|leave|attendance/,
    senderKey: "hr",
  },
  {
    pattern: /support|customer[-_ ]?care|portal|helpdesk/,
    senderKey: "support",
  },
...
function senderKeyForSource(sourceModule = "", fallback = "noreply") {
  ...
  return (
    SOURCE_SENDER_RULES.find((rule) => rule.pattern.test(source))?.senderKey ||
    fallback
  );
}
...
async function resolveSenderIdentity(
  ...
  const senderKey =
    trimText(requestedKey) || senderKeyForSource(sourceModule, fallback);
  ...
  if (!sender?.id) {
    if (requestedKey)
      throw new Error("Selected sender identity was not found or is inactive");
    return {
      senderKey,
      from: null,
      replyTo: undefined,
  ...
  if (!isAllowedFromAddress(address))
    throw new Error(`Sender ${address} is not on an allowed verified domain`);
      ...
      ? [
          {
            ...
            name: trimText(sender.reply_to_name) || name,
          },
        ]
      : undefined,
    fromEmail: address,
...
async function loadBranding(admin: any) {
  try {
    ...
    if (data) {
      const companyName =
        trimText(data.company_name) || DEFAULT_BRANDING.companyName;
      return {
        companyName,
...
function buildTestHtml(payload: Record<string, any>, branding: any) {
  const note =
    ...
    "This is a live ZeptoMail API connectivity test from EMS.";
  return wrapBrandedBody(
    "ZeptoMail API test",
    `
...
async function sendEmail(req: Request, admin: any, body: Record<string, any>) {
  ...
  if (!trimText(body.htmlBody) && !trimText(body.textBody))
...
async function callerCanChooseSender(admin: any, callerId: string) {
  ...
  const roleCodes = (roles || [])
    ...
    .filter(Boolean);
  return roleCodes.some((code: string) =>
    ["super_admin", "admin", "coo"].includes(code),
  );
}
...
function isInternalNotificationRequest(req: Request) {
  const expected = trimText(env("NOTIFICATION_CRON_SECRET"));
  const supplied = trimText(req.headers.get("x-notification-secret") || "");
...
function buildNotificationHtml(
  ...
  recipientName = "",
  branding: any = DEFAULT_BRANDING,
) {
  const accent =
    SEVERITY_ACCENT[String(event.severity || "info")] || SEVERITY_ACCENT.info;
  const title = escapeHtml(
    trimText(event.title) || `${branding.companyName} notification`,
  );
...
async function fanoutNotification(
  ...
  const caller = internal ? null : await requireActiveCaller(req, admin);
  ...
  const { data: event, error: eventError } = await admin
    ...
    .select(
      "id,module_code,event_code,category,title,message,severity,action_label,action_url,channel_plan,email_dispatched_at,created_by",
    )
    .eq("id", notificationId)
    .maybeSingle();
  if (eventError)
    throw new Error(eventError.message || "Could not load notification");
  ...
  if (!channelPlan.email) {
    return {
      ok: true,
      skipped: "email_channel_disabled",
      sent: 0,
      failed: 0,
      total: 0,
    };
  ...
  const isOwner =
    caller?.id && event.created_by && event.created_by === caller.id;
  if (!internal && !isOwner && !(await callerIsAdmin(admin, caller.id))) {
  ...
  if (recipientsError)
    throw new Error(
      recipientsError.message || "Could not resolve email recipients",
    );
  ...
  const emailEvent = {
    ...event,
    title: trimText(channelPlan.email_subject) || event.title,
    message: trimText(channelPlan.email_message) || event.message,
  };
  let sent = 0;
  let failed = 0;
  const failures: string[] = [];
  for (const recipient of list) {
    const address = normalizeEmail(recipient.destination);
    ...
    const { data: deliveryId, error: claimError } = await admin.rpc(
      "claim_notification_channel_delivery",
      {
        ...
        p_destination: address,
      },
    );
    if (claimError)
      throw new Error(claimError.message || "Could not claim email delivery");
    if (!deliveryId) continue;
    try {
      const providerResult = await sendViaZepto({
        to: [{ address, name: trimText(recipient.display_name) || address }],
        subject:
          trimText(emailEvent.title) || `${branding.companyName} notification`,
        htmlBody: buildNotificationHtml(
          emailEvent,
          recipient.display_name,
          branding,
        ),
        textBody: buildNotificationText(emailEvent, recipient.display_name),
        from: sender.from,
        replyTo: sender.replyTo,
        clientReference: `ems-notify-${notificationId}-${recipient.identity_kind}-${recipient.identity_id}`,
      });
      const requestId =
        providerResult?.request_id ||
        providerResult?.data?.[0]?.request_id ||
        null;
      await admin.rpc("complete_notification_channel_delivery", {
        p_delivery_id: deliveryId,
        p_status: "sent",
        p_provider_message_id: requestId,
        p_provider_payload: { provider: "zeptomail" },
        p_error: null,
      });
      sent += 1;
    } catch (error) {
      await admin
        .rpc("complete_notification_channel_delivery", {
          ...
          p_provider_payload: { provider: "zeptomail" },
          p_error: error?.message || "send failed",
        })
        .catch(() => null);
      failed += 1;
      if (failures.length < 5)
        failures.push(`${address}: ${error?.message || "send failed"}`);
    }
  }
...
async function googleAccessToken() {
    ...
    try {
      const decoded = rawServiceAccountJson.startsWith("base64:")
        ? new TextDecoder().decode(
            base64ToBytes(rawServiceAccountJson.slice(7)),
          )
  ...
  if (!response.ok)
    throw new Error(payload?.error_description || "Google Drive auth failed");
...
async function ensureDriveFolder(
  ...
  if (!parentId)
    throw new Error("Google Drive Email root folder is not configured");
  ...
  if (!search.ok)
    throw new Error(
      searchPayload?.error?.message || "Google Drive folder lookup failed",
    );
  ...
  if (!create.ok)
    throw new Error(
      createPayload?.error?.message || "Google Drive folder creation failed",
    );
...
async function archiveEmailAttachments(attachments: any[], subject: string) {
  ...
  try {
    ...
    const leaf = await ensureDriveFolder(
      `${stamp} ${driveFolderName(subject, "No Subject")}`,
      monthFolder.id,
      token,
    );
    ...
    return {
      archived: true,
      reason: null,
      folderLink: leaf.webViewLink || null,
      folderId: leaf.id,
      results,
    };
  } catch (error) {
    return {
      archived: false,
      reason: error?.message || "drive_archive_failed",
      folderLink: null,
      results,
    };
  }
...
function buildPortalCredentialPdf(input: Record<string, any>, branding: any) {
  ...
  if (!mobilePassword)
    throw new Error(
      "A valid registered mobile number is required for PDF protection",
    );
  ...
  const company =
    trimText(branding.companyName) || "Varada Nexus Private Limited";
  ...
  doc.text(
    isStaff ? "SECURE EMS ACCESS DOCUMENT" : "SECURE PORTAL ACCESS DOCUMENT",
    margin + 18,
    29,
  );
  ...
  doc.setFontSize(23);
  doc.text(
    isStaff ? "Your EMS credentials" : "Your portal credentials",
    margin,
    67,
  );
  ...
  doc.text(
    `Prepared for ${trimText(input.recipientName) || normalizeEmail(input.recipientEmail)}`,
    margin,
    76,
  );
  ...
  const labels = [
    isStaff ? "Workspace" : "Portal",
    "Username",
    "Initial password",
    "Login address",
  ];
  const values = [
    trimText(input.portalType),
    normalizeEmail(input.username),
    trimText(input.initialPassword),
    PUBLIC_PORTAL_LOGIN_LABEL,
  ];
  let y = 98;
  ...
  doc.text(
    isStaff
      ? "Important access conditions"
      : "Important disclaimer and identity evidence",
    margin + 7,
    218,
  );
  doc.setFont("helvetica", "normal");
  ...
  doc.text(
    doc.splitTextToSize(disclaimer, pageWidth - margin * 2 - 14),
    margin + 7,
    227,
  ...
  doc.setTextColor(100, 116, 139);
  doc.text(
    "For assistance, contact support@varadanexus.com. Varada Nexus will never ask you to share this PDF password by email.",
    margin,
    272,
  );
  doc.text(
    `Issued ${new Date().toISOString().slice(0, 10)} | ${trimText(input.portalUserCode) || (isStaff ? "Secure EMS user" : "Secure portal user")}`,
    margin,
    280,
  );
...
function buildPortalCredentialEmailHtml(input: Record<string, any>) {
  ...
  const portal = escapeHtml(
    trimText(input.portalType) || "Varada Nexus portal",
  );
...
async function sendPortalCredentials(
  req: Request,
  admin: any,
  body: Record<string, any>,
) {
  ...
  if (username !== recipientEmail)
    throw new Error("Portal username must be the recipient email address");
  if (initialPassword.length < 8)
    throw new Error("Initial password must be at least 8 characters");
  if (!normalizeMobilePassword(registeredMobile))
    throw new Error("A valid registered mobile number is required");

  const input = {
    recipientEmail,
    recipientName:
      trimText(body.recipientName) ||
      trimText(body.linkedEntityName) ||
      recipientEmail,
    username,
    initialPassword,
    registeredMobile,
  ...
  const result = await sendModuleEmail(req, admin, {
    ...
    sourceEvent: isResend
      ? "portal_credentials_resent"
      : "portal_credentials_created",
  });
...
function buildUserCredentialEmailHtml(input: Record<string, any>) {
  ...
  const division = escapeHtml(
    trimText(input.divisionName) || "Assigned workspace",
  );
  return `
    <p style="margin:0 0 14px;">Hello ${recipient},</p>
...
function buildUserCredentialEmailText(input: Record<string, any>) {
  return [
    ...
    "",
    "For help, contact support@varadanexus.com.",
    "",
    "Varada Nexus Private Limited",
  ].join("\n");
}
...
async function sendUserCredentials(
  ...
  const result = await sendModuleEmail(req, admin, {
    to: [{ address: recipientEmail, name: input.recipientName }],
    subject: isResend
      ? "Your updated Varada Nexus EMS credentials"
      : "Your Varada Nexus EMS access",
    ...
    sourceModule: "users",
    sourceEvent: isResend
      ? "ems_user_credentials_resent"
      : "ems_user_credentials_created",
  });
  return { ...result, senderKey: "noreply", recipientEmail };
}
...
async function listEmailDirectory(req: Request, admin: any) {
  ...
  const users = (data || [])
    ...
    .map((row: any) => ({
      id: row.id,
      name: trimText(row.display_name) || row.email,
...
async function listEmailWorkspaceData(req: Request, admin: any) {
  ...
  const stats = {
    sent: outbox.filter((r: any) => String(r.status).toLowerCase() === "sent")
      .length,
    failed: outbox.filter(
      (r: any) => String(r.status).toLowerCase() === "failed",
    ).length,
...
async function sendModuleEmail(
  req: Request,
  admin: any,
  body: Record<string, any>,
) {
  ...
  if (
    (isManualCompose || trimText(body.senderKey)) &&
    !(await callerCanChooseSender(admin, caller.id))
  ) {
    throw new Error(
      "Only Admin and COO users can compose using departmental sender identities",
    );
  }
  ...
  if (Array.isArray(body.to))
    body.to.forEach((item: any) =>
      pushRecipient(item.address || item.email, item.name),
    );
  ...
  const sender = await resolveSenderIdentity(
    ...
    trimText(body.senderKey) || null,
    sourceModule,
    sourceModule === "email-compose" ? "admin" : "noreply",
  );
  ...
  const totalRaw = rawAttachments.reduce(
    (sum: number, a: any) => sum + Math.floor(a.base64.length * 0.75),
    0,
  );
  if (totalRaw > 10 * 1024 * 1024)
  ...
  const zeptoAttachments = rawAttachments.map((a: any) => ({
    content: a.base64,
...
async function inboundEmail(admin: any, body: Record<string, any>) {
  ...
  if (!fromEmail || !fromEmail.includes("@"))
    throw new Error("A valid fromEmail is required");
  const row = {
    from_email: fromEmail,
...
async function listEmailSenders(req: Request, admin: any) {
  const caller = await requireActiveCaller(req, admin);
  if (!(await callerCanChooseSender(admin, caller.id)))
    throw new Error("Admin or COO permission required");
  const { data } = await admin
    .from("email_senders")
    .select("*")
...
async function saveEmailSender(
  ...
  if (!isAllowedFromAddress(fromEmail))
    throw new Error(`${fromEmail} is not on an allowed verified domain`);
  ...
  const { data, error } = await admin
    .from("email_senders")
    .upsert(row, { onConflict: "sender_key" })
    .select()
    .maybeSingle();
  if (error) throw new Error(error.message || "Could not save sender identity");
  await recordAudit(admin, caller, "email_sender_saved", {
    senderKey,
    fromEmail,
  });
...
async function deleteEmailSender(
  req: Request,
  admin: any,
  body: Record<string, any>,
) {
  ...
  if (error)
    throw new Error(error.message || "Could not delete sender identity");
...
async function saveEmailBranding(
  ...
  const caller = await requireActiveCaller(req, admin);
  const companyName =
    trimText(body.companyName) || DEFAULT_BRANDING.companyName;
  const row = {
    id: 1,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "send_test_email")
      return json(await sendTestEmail(req, admin, body));
    if (action === "send_email") return json(await sendEmail(req, admin, body));
    if (action === "fanout_notification")
      return json(await fanoutNotification(req, admin, body));
    if (action === "list_email_workspace_data")
      return json(await listEmailWorkspaceData(req, admin));
    if (action === "send_module_email")
      return json(await sendModuleEmail(req, admin, body));
    if (action === "send_portal_credentials")
      return json(await sendPortalCredentials(req, admin, body));
    if (action === "send_user_credentials")
      return json(await sendUserCredentials(req, admin, body));
    if (action === "list_email_history")
      return json(await listEmailHistory(req, admin));
    if (action === "list_email_inbound")
      return json(await listEmailInbound(req, admin));
    if (action === "mark_inbound_read")
      return json(await markInboundRead(req, admin, body));
    if (action === "list_email_templates")
      return json(await listEmailTemplates(req, admin));
    if (action === "save_email_template")
      return json(await saveEmailTemplate(req, admin, body));
    if (action === "delete_email_template")
      return json(await deleteEmailTemplate(req, admin, body));
    if (action === "list_email_directory")
      return json(await listEmailDirectory(req, admin));
    if (action === "list_email_senders")
      return json(await listEmailSenders(req, admin));
    if (action === "save_email_sender")
      return json(await saveEmailSender(req, admin, body));
    if (action === "delete_email_sender")
      return json(await deleteEmailSender(req, admin, body));
    if (action === "get_email_branding")
      return json(await getEmailBranding(req, admin));
    if (action === "save_email_branding")
      return json(await saveEmailBranding(req, admin, body));
    if (action === "inbound_email")
      return json(await inboundEmail(admin, body));
    return json({ error: "Unsupported action" }, 400);
  } catch (error) {
    return json(
      { error: error?.message || "Email integration request failed" },
      400,
    );
  }
});
