-- Sprint 9C.6B Stabilization follow-up
-- Because the earlier stabilization migration had already been recorded remotely,
-- this migration reapplies the intended two-stage workflow as a forward-only fix.
...
begin
  ...
  if p_document_family = 'CLIENT_BILL' and p_source_table = 'transport_client_bills' then
    select b.division_id, b.bill_date, b.bill_date, b.bill_no, b.status, b.transport_client_id,
           coalesce(tc.company_name, tc.name), round(coalesce(b.gross_total,0)::numeric,2),
           round(coalesce(b.taxable_value,0)::numeric,2), round(coalesce(b.gst_amount,0)::numeric,2),
    ...
    select i.division_id, i.invoice_date, i.invoice_date, i.invoice_no, i.status, i.transport_client_id,
           coalesce(tc.company_name, tc.name), round(coalesce(i.invoice_total,0)::numeric,2),
           round(coalesce(i.taxable_value,0)::numeric,2), round(coalesce(i.gst_amount,0)::numeric,2),
    ...
    select n.division_id, n.credit_note_date, n.credit_note_date, n.credit_note_no, n.status, n.transport_client_id,
           coalesce(tc.company_name, tc.name), round(coalesce(n.credit_note_amount,0)::numeric,2),
           0::numeric(14,2), 0::numeric(14,2), round(coalesce(n.credit_note_amount,0)::numeric,2)
    into v_division_id, v_document_date, v_effective_date, v_source_no, v_status, v_counterparty_id,
         v_counterparty_name, v_gross, v_taxable, v_tax, v_net
    ...
    select r.division_id, r.receipt_date, r.receipt_date, r.receipt_no, r.status, r.transport_client_id,
           coalesce(tc.company_name, tc.name), round(coalesce(r.amount_received,0)::numeric,2),
           0::numeric(14,2), 0::numeric(14,2), round(coalesce(r.amount_received,0)::numeric,2)
    into v_division_id, v_document_date, v_effective_date, v_source_no, v_status, v_counterparty_id,
         v_counterparty_name, v_gross, v_taxable, v_tax, v_net
    ...
    select s.division_id, s.statement_date, s.statement_date, s.statement_no, s.status, s.transport_transporter_id,
           tt.name, round(coalesce(s.gross_payable_total,0)::numeric,2),
           0::numeric(14,2), 0::numeric(14,2), round(coalesce(s.net_payable_total,0)::numeric,2)
    into v_division_id, v_document_date, v_effective_date, v_source_no, v_status, v_counterparty_id,
         v_counterparty_name, v_gross, v_taxable, v_tax, v_net
    ...
    select p.division_id, p.payment_date, p.payment_date, p.payment_no, p.status, p.transport_transporter_id,
           tt.name, round(coalesce(p.amount_paid,0)::numeric,2),
           0::numeric(14,2), 0::numeric(14,2), round(coalesce(p.amount_paid,0)::numeric,2)
    into v_division_id, v_document_date, v_effective_date, v_source_no, v_status, v_counterparty_id,
         v_counterparty_name, v_gross, v_taxable, v_tax, v_net
  ...
  insert into public.financial_documents (
    ...
    p_document_family, 'transportation', p_source_table, p_source_id, v_source_no,
    v_division_id, v_counterparty_dimension_id, v_project_dimension_id, v_profit_center_dimension_id,
    'ready_for_posting', v_document_date, coalesce(v_effective_date, v_document_date), v_gross, v_taxable, v_tax, v_net,
    v_actor_app_user_id, now()
  )
  ...
  insert into public.posting_queue (financial_document_id, queue_status, requested_by, processed_by, processed_at, last_error)
  values (v_financial_document_id, 'ready_to_post', v_actor_app_user_id, null, null, null)
  ...
  perform public.log_central_accounts_audit_event(
    'transport_bridge_staged', p_document_family, p_source_id::text, v_financial_document_id, null, v_queue_id, null, v_actor_app_user_id,
    jsonb_build_object('source_module','transportation','source_table',p_source_table,'source_no',v_source_no,'stage','ready_for_posting')
  );

  return v_financial_document_id;
exception when others then
  perform public.log_central_accounts_audit_event(
    'transport_bridge_failed', coalesce(p_document_family,'UNKNOWN'), coalesce(p_source_id::text,'UNKNOWN'), v_financial_document_id, null, v_queue_id, null, v_actor_app_user_id,
    jsonb_build_object('source_module','transportation','source_table',p_source_table,'source_no',v_source_no,'error',sqlerrm)
  );
  return v_financial_document_id;
end;
$$;
...
begin
  ...
  if exists (select 1 from public.document_postings dp where dp.financial_document_id = v_fd.id and dp.posting_status = 'posted') then
  ...
  insert into public.journal_entries (
    ...
    coalesce(v_fd.effective_date, v_fd.document_date), 'transportation', v_fd.source_document_id, v_fd.document_family, v_fd.division_id,
  ...
  if v_fd.document_family = 'CLIENT_BILL' then
    ...
    set original_amount = excluded.original_amount, open_amount = excluded.open_amount, due_date = excluded.due_date, status = excluded.status, updated_at = now();
    ...
    if v_related_source_id is not null then
      ...
      where fd.source_module = 'transportation' and fd.source_table = 'transport_client_bills' and fd.source_document_id = v_related_source_id and fd.document_family = 'CLIENT_BILL'
      limit 1;
    ...
    if v_related_source_id is not null then
      ...
      where fd.source_module = 'transportation' and fd.source_table = 'transport_transporter_statements' and fd.source_document_id = v_related_source_id and fd.document_family = 'TRANSPORTER_STATEMENT'
      limit 1;
      if v_open_item_id is not null then
  ...
  update public.document_postings set posting_status = 'posted', journal_entry_id = v_journal_id, posted_by = v_actor_app_user_id, posted_at = now(), updated_at = now() where id = v_posting_id;
  update public.posting_queue set queue_status = 'posted', processed_by = v_actor_app_user_id, processed_at = now(), updated_at = now(), last_error = null where id = v_queue_id;
  update public.financial_documents set status = 'posted', posted_by = v_actor_app_user_id, posted_at = now(), updated_at = now() where id = v_fd.id;

  perform public.log_central_accounts_audit_event(
    'transport_posted', v_fd.document_family, v_fd.source_document_id::text, v_fd.id, v_journal_id, v_queue_id, v_period_id, v_actor_app_user_id,
...
exception when others then
  ...
  perform public.log_central_accounts_audit_event(
    'transport_post_failed', coalesce(v_fd_document_family,'UNKNOWN'), coalesce(v_fd_source_document_id::text, p_financial_document_id::text), p_financial_document_id,
    v_journal_id, v_queue_id, v_period_id, v_actor_app_user_id, jsonb_build_object('error',sqlerrm)
  );
  raise;
end;
$$;
...
begin
  ...
  delete from public.receivable_allocations ra using public.financial_documents fd where ra.applied_document_id = fd.id and fd.source_module = 'transportation';
  delete from public.payable_allocations pa using public.financial_documents fd where pa.applied_document_id = fd.id and fd.source_module = 'transportation';
  delete from public.receivable_open_items roi using public.financial_documents fd where roi.financial_document_id = fd.id and fd.source_module = 'transportation';
  delete from public.payable_open_items poi using public.financial_documents fd where poi.financial_document_id = fd.id and fd.source_module = 'transportation';
  delete from public.journal_lines jl using public.journal_entries je join public.financial_documents fd on fd.id = je.financial_document_id where jl.journal_entry_id = je.id and fd.source_module = 'transportation';
  update public.document_postings dp set journal_entry_id = null, reversal_journal_entry_id = null from public.financial_documents fd where dp.financial_document_id = fd.id and fd.source_module = 'transportation';
  delete from public.journal_entries je using public.financial_documents fd where je.financial_document_id = fd.id and fd.source_module = 'transportation';
  delete from public.document_postings dp using public.financial_documents fd where dp.financial_document_id = fd.id and fd.source_module = 'transportation';

  update public.financial_documents
  set status = 'ready_for_posting', posted_by = null, posted_at = null, updated_at = now()
  ...
  on conflict (financial_document_id)
  do update set queue_status = 'ready_to_post', processed_by = null, processed_at = null, last_error = null, updated_at = now();

  for r in select fd.id, fd.document_family, fd.source_document_id from public.financial_documents fd where fd.source_module = 'transportation'
  loop
    perform public.log_central_accounts_audit_event(
      'transport_bridge_restaged', r.document_family, r.source_document_id::text, r.id, null, null, null, public.current_app_user_id(),
      jsonb_build_object('reason','approval_not_posting_stabilization_followup')
    );
  end loop;
end $$;