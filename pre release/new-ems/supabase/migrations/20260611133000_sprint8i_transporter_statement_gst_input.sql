-- Sprint 8I: Transporter statement GST input support and payable totals extension

alter table public.transport_transporter_statements
  add column if not exists gst_input_applicable boolean not null default false,
  add column if not exists gst_input_mode text,
  add column if not exists gst_input_percentage numeric(5,2),
  add column if not exists gst_input_amount numeric(18,2) not null default 0;

update public.transport_transporter_statements
set gst_input_applicable = coalesce(gst_input_applicable, false),
    gst_input_amount = coalesce(gst_input_amount, 0)
where gst_input_applicable is null
   or gst_input_amount is null;

do $$ begin
  alter table public.transport_transporter_statements
    add constraint chk_transport_transporter_statements_gst_input_mode
    check (gst_input_mode is null or gst_input_mode in ('INCLUSIVE', 'EXCLUSIVE'));
exception when duplicate_object then null; end $$;

do $$ begin
  alter table public.transport_transporter_statements
    add constraint chk_transport_transporter_statements_gst_input_percentage
    check (gst_input_percentage is null or gst_input_percentage in (0,5,12,18,28));
exception when duplicate_object then null; end $$;

do $$ begin
  alter table public.transport_transporter_statements
    add constraint chk_transport_transporter_statements_gst_input_amount
    check (gst_input_amount >= 0);
exception when duplicate_object then null; end $$;

do $$ begin
  alter table public.transport_transporter_statements
    add constraint chk_transport_transporter_statements_gst_input_consistency
    check (
      (gst_input_applicable = false and gst_input_mode is null and gst_input_percentage is null and coalesce(gst_input_amount, 0) = 0)
      or
      (gst_input_applicable = true and gst_input_mode in ('INCLUSIVE', 'EXCLUSIVE') and gst_input_percentage in (0,5,12,18,28) and coalesce(gst_input_amount, 0) >= 0)
    );
exception when duplicate_object then null; end $$;
...
create or replace function public.create_transport_transporter_statement(
  ...
  p_gst_input_applicable boolean default false,
  p_gst_input_mode text default null,
  p_gst_input_percentage numeric default null
)
returns table (
  statement_id uuid,
  statement_no text,
  ...
  gst_input_applicable boolean,
  gst_input_mode text,
  gst_input_percentage numeric,
  gst_input_amount numeric,
...
declare
  ...
  v_gst_input_applicable boolean := coalesce(p_gst_input_applicable, false);
  v_gst_input_mode text := case when coalesce(p_gst_input_applicable, false) then nullif(upper(btrim(coalesce(p_gst_input_mode, ''))), '') else null end;
  v_gst_input_percentage numeric(5,2) := case when coalesce(p_gst_input_applicable, false) then p_gst_input_percentage else null end;
  v_gross_total numeric(14,2) := 0;
  v_support_total numeric(14,2) := 0;
  v_pre_gst_base_amount numeric(14,2) := 0;
  v_gst_input_amount numeric(14,2) := 0;
  v_net_payable_total numeric(14,2) := 0;
...
begin
  ...
  if v_gst_input_applicable then
    if v_gst_input_mode not in ('INCLUSIVE', 'EXCLUSIVE') then raise exception 'Invalid GST input mode'; end if;
    if v_gst_input_percentage not in (0,5,12,18,28) then raise exception 'Invalid GST input percentage'; end if;
  else
    v_gst_input_mode := null;
    v_gst_input_percentage := null;
  end if;
  ...
  v_pre_gst_base_amount := round((coalesce(v_gross_total, 0) - coalesce(v_support_total, 0) - coalesce(v_penalty_amount, 0))::numeric, 2);
  if v_pre_gst_base_amount < 0 then raise exception 'net base amount cannot be negative'; end if;

  if v_gst_input_applicable then
    if v_gst_input_mode = 'INCLUSIVE' then
      if coalesce(v_gst_input_percentage, 0) = 0 then
        v_gst_input_amount := 0;
        v_net_payable_total := round(v_pre_gst_base_amount::numeric, 2);
      else
        v_gst_input_amount := round((v_pre_gst_base_amount - (v_pre_gst_base_amount / (1 + (v_gst_input_percentage / 100.0))))::numeric, 2);
        v_net_payable_total := round(v_pre_gst_base_amount::numeric, 2);
      end if;
    else
      v_gst_input_amount := round((v_pre_gst_base_amount * (coalesce(v_gst_input_percentage, 0) / 100.0))::numeric, 2);
      v_net_payable_total := round((v_pre_gst_base_amount + v_gst_input_amount)::numeric, 2);
    end if;
  else
    v_gst_input_amount := 0;
    v_net_payable_total := round(v_pre_gst_base_amount::numeric, 2);
  ...
  with eligible as (
    ...
    insert into public.transport_transporter_statements (
      ...
      gross_payable_total, support_deduction_total, penalty_amount, penalty_reason,
      gst_input_applicable, gst_input_mode, gst_input_percentage, gst_input_amount,
      net_payable_total, remarks
    )
    select p_division_id,
           ...
           v_penalty_amount,
           v_penalty_reason,
           v_gst_input_applicable,
           v_gst_input_mode,
           v_gst_input_percentage,
           v_gst_input_amount,
           v_net_payable_total,
  ...
  select s.id as statement_id,
         ...
         s.gst_input_applicable as gst_input_applicable,
         s.gst_input_mode as gst_input_mode,
         s.gst_input_percentage as gst_input_percentage,
         s.gst_input_amount as gst_input_amount,
