-- Sprint 14B: balanced manual voucher engine with maker-checker posting.

insert into public.permissions (module_code, action_code, label, is_active)
select 'central-accounts-vouchers', action_code, 'Central Accounts — Manual Vouchers (' || action_code || ')', true
from (values ('view'),('create'),('edit'),('approve'),('post'),('reverse'),('export')) a(action_code)
...
create table if not exists public.accounting_vouchers (
  ...
  reversal_of_voucher_id uuid references public.accounting_vouchers(id) on delete restrict,
  reversed_by_voucher_id uuid references public.accounting_vouchers(id) on delete restrict,
  rejection_reason text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check (voucher_type in ('journal','payment','receipt','contra','debit_note','credit_note','accrual','provision','prepaid','adjustment','opening')),
  check (status in ('draft','submitted','approved','posted','rejected','reversed')),
...
grant select,insert,update on public.accounting_vouchers,public.accounting_voucher_lines to authenticated;
revoke all on public.accounting_vouchers,public.accounting_voucher_lines from anon;

create policy accounting_vouchers_select on public.accounting_vouchers for select to authenticated
using (public.is_super_admin() or public.has_role_code('admin') or public.has_permission('central-accounts-vouchers','view'));
create policy accounting_voucher_lines_select on public.accounting_voucher_lines for select to authenticated
using (public.is_super_admin() or public.has_role_code('admin') or public.has_permission('central-accounts-vouchers','view'));
...
create index if not exists idx_accounting_vouchers_date_status on public.accounting_vouchers(voucher_date desc,status);
