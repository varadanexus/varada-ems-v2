...
--
-- Changes:
-- 1. mask_email / mask_phone — safe display helpers (no sensitive data)
-- 2. transport_portal_login   — extend identifier lookup to email + phone (fixes 400)
-- 3. interiors_portal_list_my_access — new RPC using auth.uid() (interiors uses Supabase Auth)
-- 4. unified_login_lookup     — safe cross-system identifier lookup for anon login page
--
-- Security constraints preserved:
-- • No password_hash, encrypted_password_vault, reset_token_hash, session_token exposed
-- • All portal tables remain default-deny RLS; access only via SECURITY DEFINER functions
-- • unified_login_lookup does NOT verify passwords — identifier existence only
-- • interiors_portal_list_my_access uses auth.uid() — requires valid Supabase Auth JWT

-- ============================================================
-- 1) Safe masking helpers
-- ============================================================
...
-- ============================================================
-- 2) Fix transport_portal_login: extend identifier to email + phone
--    Root cause of 400: lookup was username-only; transporter entered email/phone.
-- ============================================================
...
begin
  ...
  -- Extended lookup: username OR email OR phone (was username-only — caused 400 for email/phone users)
  select * into v_user
...
-- ============================================================
-- 3) interiors_portal_list_my_access
--    Interiors portal uses Supabase Auth (interior_client_portal_users.auth_user_id).
--    No session_token parameter — auth context comes from the JWT via auth.uid().
--    Called after client.auth.signInWithPassword() succeeds on the frontend.
-- ============================================================
...
-- ============================================================
-- 4) unified_login_lookup
--    Safe identifier lookup for the anon login page.
--    Returns which auth systems contain this identifier.
--    NEVER returns password_hash, encrypted_password_vault,
--    reset_token_hash, session_token, or any auth secret.
-- ============================================================
...
begin
  ...
  -- EMS Staff: app_users matched by email only (EMS never uses username/phone to identify)
  return query
  select
    'ems'::text,
    'EMS Staff'::text,
  ...
  -- Interiors Client Portal: email or phone (no username column)
  return query
  select
    'interiors'::text,
