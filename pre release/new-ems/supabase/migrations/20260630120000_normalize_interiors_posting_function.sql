-- Migration history normalization (Sprint 10D follow-up).
--
-- Context: 20260619122500_sync_user_access_mappings.sql, 20260623111000_fix_interiors_posting_authorization.sql,
-- and 20260623134000_add_client_role_for_interiors_portal.sql exist in new-ems/supabase/migrations but were
-- never recorded as applied in the linked project's remote migration history table (they were originally run
-- against this same database via an earlier root-level `supabase/` CLI context, before
-- "Repair Supabase migration source of truth" consolidated everything into new-ems/supabase/migrations).
--
-- Live verification before writing this file confirmed all three migrations' effects are already present on
-- the linked database: public.sync_user_access_mappings exists, roles.code='client' exists with
-- interiors-client-portal:view granted, and public.execute_central_accounts_interiors_posting already carries
-- the has_permission('central-accounts-posting-queue','post') / can_emergency_post_central_accounts()
-- authorization check from 20260623111000 (confirmed via a byte-level pg_get_functiondef comparison against
-- the live database, not assumed).
--
-- This migration does NOT change behavior. It exists solely to make
-- public.execute_central_accounts_interiors_posting(uuid) authoritative in forward migration order, so the
-- out-of-order 20260623111000 file does not need to be replayed via `db push --include-all`. The body below
-- is copied verbatim from the current active migration chain (new-ems/supabase/migrations), which a live
-- pg_get_functiondef comparison confirmed is identical to what is already running in production.
--
-- Preserved unchanged: posting-permission + emergency-access authorization, duplicate-posting protection,
-- posting_queue lifecycle updates, financial_documents status update, document_postings creation,
-- journal_entries creation, journal_lines creation, receivable_open_items creation (no payable_open_items —
-- Interiors bills are receivable-only), audit logging on both success and failure paths, and the
-- EXCEPTION WHEN OTHERS handler. No GST invoice logic is introduced; none existed before.
...
grant execute on function public.execute_central_accounts_interiors_posting(uuid) to authenticated;
