...
const PAGE_META = {
  ...
  "vendor-payments": ["Vendor Payments", "Accounts-owned vendor bills, approvals, advice and ledger"],
...
const state = { view: "overview", boot: null, project: null, scopes: [], packages: [], items: [], licenses: [], vendors: [], assignments: [], invoices: [], credits: [], queries: [], messages: [], documents: [], emails: [], whatsapp: [] };
...
async function loadView() {
  ...
  if (state.view === "overview") {
    ...
    [state.assignments, state.packages] = await Promise.all([
      query("hospital_project_vendors", "*,hospital_vendors(id,legal_name,vendor_code)").then((rows) => rows.filter((x) => x.project_id === id)),
      query("hospital_work_packages", "*,hospital_vendors(id,legal_name,vendor_code)").then((rows) => rows.filter((x) => x.project_id === id))
    ]);
...
function clientPaymentsPage() { return `${projectHeader()}<section class="hp-card"><div class="hp-actions"><div><h3>Client Payments</h3><p class="muted">Invoices and numbering remain in centralized Hospital billing.</p></div><a class="btn" href="${ROUTES.HOSPITAL_BILLING}?project_id=${encodeURIComponent(projectId)}">Open centralized billing</a></div>${table(["Invoice", "Issue / due", "Total", "Paid", "Credited", "Balance", "Status"], state.invoices.map((x) => `<tr><td><strong>${esc(x.invoice_number)}</strong></td><td>${date(x.issue_date)}<br><small>${date(x.due_date)}</small></td><td>${money(x.total_amount)}</td><td>${money(x.paid_amount)}</td><td>${money(x.credited_amount)}</td><td>${money(x.balance_amount)}</td><td>${esc(label(x.status))}</td></tr>`).join(""), "No client invoices")}</section>`; }
function vendorPaymentsPage() { return `${projectHeader()}<section class="hp-card"><h3>Vendor Payments</h3><p>Vendor bills, approvals, payment advice, payment history and vendor ledgers remain owned by Central Accounts.</p><div class="hp-workspace-links"><a class="hp-workspace-link" href="${ROUTES.CENTRAL_ACCOUNTS_FINANCIAL_DOCUMENTS}?source_module=hospital-projects&project_id=${encodeURIComponent(projectId)}"><strong>Vendor Bills</strong><small>Financial document intake</small></a><a class="hp-workspace-link" href="${ROUTES.CENTRAL_ACCOUNTS_POSTING_QUEUE}?source_module=hospital-projects&project_id=${encodeURIComponent(projectId)}"><strong>Approvals</strong><small>Posting and approval queue</small></a><a class="hp-workspace-link" href="${ROUTES.CENTRAL_ACCOUNTS_PAYABLES}?source_module=hospital-projects&project_id=${encodeURIComponent(projectId)}"><strong>Payment Advice & History</strong><small>Accounts payable operations</small></a><a class="hp-workspace-link" href="${ROUTES.CENTRAL_ACCOUNTS_REPORTING}?source_module=hospital-projects&project_id=${encodeURIComponent(projectId)}"><strong>Vendor Ledger</strong><small>Central Accounts reporting</small></a></div></section><section class="hp-card"><h3>Project commercial commitments</h3>${table(["Vendor", "Package", "Awarded / sell value", "Status"], [...state.assignments.map((x) => ({ vendor: x.hospital_vendors?.legal_name, title: x.package_name, value: x.awarded_value, status: x.status })), ...state.packages.map((x) => ({ vendor: x.hospital_vendors?.legal_name, title: x.title, value: x.client_price, status: x.status }))].map((x) => `<tr><td>${esc(x.vendor || "Unassigned")}</td><td>${esc(x.title)}</td><td>${money(x.value)}</td><td>${esc(label(x.status))}</td></tr>`).join(""), "No vendor commitments")}</section>`; }
