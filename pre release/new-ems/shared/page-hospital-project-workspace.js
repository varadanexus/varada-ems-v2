...
const state = { view: "overview", boot: null, project: null, scopes: [], packages: [], items: [], licenses: [], vendors: [], assignments: [], invoices: [], credits: [], vendorBills: [], queries: [], messages: [], documents: [], emails: [], whatsapp: [], equipmentEditingId: null };
...
async function prefetchProject() {
  ...
  const { data, error } = await db.from("hospital_projects")
    .select("*,hospital_clients(id,hospital_name,client_code,email,phone)")
...
async function loadView() {
  ...
  if (state.view === "overview") {
    ...
    [state.items, state.vendors] = await Promise.all([
...
function equipmentPage(title) {
  ...
  return `${projectHeader()}${canSave ? `<section class="hp-card"><h3>${editing ? "Edit" : "Add"} ${esc(title)} item</h3><p class="muted">${editing ? "Update the saved equipment record. Its project assignment cannot be changed." : "Create the equipment requirement without leaving this project workspace."}</p><form id="projectEquipmentForm" class="hp-form"><label>Equipment<input name="equipment_name" value="${esc(editing?.equipment_name || "")}" required></label><label>Make / model<input name="make_model" value="${esc(editing?.make_model || "")}"></label><label>Vendor<select name="vendor_id">${vendorSelect}</select></label><label>Quantity<input name="quantity" type="number" min=".001" step=".001" value="${Number(editing?.quantity || 1)}" required></label><label>Vendor unit price<input name="vendor_unit_cost" type="number" min="0" step=".01" value="${Number(editing?.vendor_unit_cost || 0)}"></label><label>Landed unit cost<input name="landed_unit_cost" type="number" min="0" step=".01" value="${Number(editing?.landed_unit_cost || 0)}"></label><label>Hospital unit selling price<input name="client_unit_price" type="number" min="0" step=".01" value="${Number(editing?.client_unit_price || 0)}"></label><label>Status<select name="status">${statusSelect}</select></label><label class="wide">Specification<textarea name="specification">${esc(editing?.specification || "")}</textarea></label><div class="hp-actions"><button class="btn">${editing ? "Update equipment" : "Add equipment"}</button>${editing ? `<button class="btn" id="cancelEquipmentEdit" type="button">Cancel edit</button>` : ""}</div></form></section>` : ""}<section class="hp-card"><h3>${esc(title)}</h3><p class="muted">Project-scoped equipment and procurement register.</p>${table(["Equipment", "Vendor", "Quantity", "Landed cost", "Hospital price", "Margin", "Status", ...(actions ? ["Actions"] : [])], state.items.map((x) => `<tr><td><strong>${esc(x.equipment_name)}</strong><br><small>${esc(x.make_model || "")}</small></td><td>${esc(x.hospital_vendors?.legal_name || "Unassigned")}</td><td>${Number(x.quantity) || 0}</td><td>${money(x.total_vendor_cost)}</td><td>${money(x.total_client_price)}</td><td>${money(x.margin_amount)}</td><td>${esc(label(x.status))}</td>${actions ? `<td><div class="hp-actions">${can(PERMISSIONS.EDIT) ? `<button class="btn btn-sm" type="button" data-edit-equipment="${x.id}">Edit</button>` : ""}${can(PERMISSIONS.DELETE) ? `<button class="btn btn-sm" type="button" data-delete-equipment="${x.id}">Delete</button>` : ""}</div></td>` : ""}</tr>`).join(""), "No equipment requirements")}</section>`;
}
...
function bind() {
  ...
  document.getElementById("projectEquipmentForm")?.addEventListener("submit", async (event) => { event.preventDefault();try{const payload=values(event.currentTarget),editingId=state.equipmentEditingId;for(const key of ["quantity","vendor_unit_cost","landed_unit_cost","client_unit_price"])payload[key]=Number(payload[key]||0);if(editingId){const before=state.items.find((x)=>x.id===editingId);const {data,error}=await db.from("hospital_procurement_items").update(payload).eq("id",editingId).eq("project_id",projectId).select("id").maybeSingle();if(error)throw error;if(!data)throw new Error("Equipment record was not found or cannot be edited.");await logAuditEvent("hospital_equipment_item_updated",{moduleCode:MODULES.HOSPITAL_PROJECTS,entityType:"hospital_procurement_items",entityId:editingId,beforeData:before,afterData:payload});state.equipmentEditingId=null;showToast("Equipment item updated.",TOAST_TYPES.SUCCESS);}else{payload.project_id=projectId;const {data,error}=await db.from("hospital_procurement_items").insert(payload).select("id").single();if(error)throw error;await logAuditEvent("hospital_equipment_item_created",{moduleCode:MODULES.HOSPITAL_PROJECTS,entityType:"hospital_procurement_items",entityId:data?.id,afterData:payload});showToast("Equipment item added.",TOAST_TYPES.SUCCESS);}await loadView();render();}catch(error){showToast(error.message,TOAST_TYPES.ERROR);} });
