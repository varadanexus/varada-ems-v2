...
function renderShell(divisionLabel) {
  return `
    <style>
      ...
      .ledger-kpi-card{padding:1rem;border:1px solid rgba(230,200,126,.16);border-radius:16px;background:linear-gradient(145deg,rgba(230,200,126,.045),#07080d 62%,#050609);box-shadow:inset 0 1px rgba(255,255,255,.015)}
      .ledger-kpi-card label{display:block;font-size:.76rem;color:#e6c87e;text-transform:uppercase;margin-bottom:.45rem}
      .ledger-kpi-card strong{display:block;font-size:1.1rem;color:#f7f4ec}
      .ledger-kpi-card small{display:block;margin-top:.35rem;color:#9b9788}
      .ledger-filter-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:.85rem 1rem;align-items:end}
      .ledger-register-table{width:100%;border-collapse:collapse;background:#07080d;color:#c9c5b8}
      .ledger-register-table th,.ledger-register-table td{padding:.62rem .5rem;text-align:left;border-bottom:1px solid rgba(230,200,126,.1);font-size:.9rem}
      .ledger-register-table th{font-size:.76rem;text-transform:uppercase;color:#e6c87e;background:#0b0c10;position:sticky;top:0}
      .ledger-register-table tbody tr:nth-child(even) td{background:#0a0b0f}
      .ledger-register-table tbody tr:nth-child(odd) td{background:#07080d}
      .amount-debit,.balance-positive{color:#86efac;font-weight:700}.amount-credit,.balance-negative{color:#fca5a5;font-weight:700}
      .reference-cell button{padding:0;border:none;background:none;color:#e6c87e;font-weight:700;cursor:pointer;text-decoration:underline}
      .ledger-actions{display:flex;gap:.75rem;flex-wrap:wrap;align-items:center}
      .ledger-modal[hidden]{display:none}.ledger-modal{position:fixed;inset:0;z-index:3000;padding:1rem;display:flex;align-items:center;justify-content:center;background:rgba(4,5,8,.88);backdrop-filter:blur(12px)}
      .ledger-modal-panel{width:min(1200px,95vw);max-height:85vh;overflow-y:auto;background:linear-gradient(145deg,#0b0c10,#050609);color:#c9c5b8;border:1px solid rgba(230,200,126,.2);border-radius:18px;padding:1rem;box-shadow:0 28px 80px rgba(0,0,0,.65)}.ledger-detail-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.85rem;margin-bottom:1rem}.ledger-detail-box{padding:.85rem 1rem;border-radius:14px;background:#07080d;border:1px solid rgba(230,200,126,.14)}.ledger-detail-box label{display:block;font-size:.78rem;color:#9b9788;text-transform:uppercase;margin-bottom:.35rem}.ledger-detail-box strong{font-size:1rem;color:#f7f4ec}.ledger-accounting-toggle{margin-bottom:1rem}
      .ledger-modal-panel .table-shell{max-height:300px;overflow:auto;background:#07080d;border:1px solid rgba(230,200,126,.14);border-radius:12px}
      .ledger-modal-panel table{background:#07080d;color:#c9c5b8}.ledger-modal-panel th,.ledger-modal-panel td{padding:.65rem .5rem;border-bottom:1px solid rgba(230,200,126,.1)}
