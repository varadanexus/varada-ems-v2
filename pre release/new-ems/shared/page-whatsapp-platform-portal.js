...
function billingView(view = "billing") {
  ...
  const addonSubscriptionsSection = openAddonSubscriptions.length ? `<section class="wp-card wp-billing-addon-subscriptions"><div class="wp-card-heading"><div><span class="wp-card-eyebrow">Add-ons</span><h2>Your add-ons</h2><p>Only successfully authorized add-ons are activated. Retry or cancel incomplete payments here.</p></div><span class="wp-billing-addon-count ${activeAddonSubscriptions.length ? "" : "is-pending"}">${addonCountLabel}</span></div><div class="wp-billing-addon-subscription-list">${addonSubscriptionCards}</div></section>` : "";
