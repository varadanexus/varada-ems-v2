...
function renderPage(data = {}) {
  ...
  renderModuleContent(`
    <style>
      .mcd-hero{padding:1.35rem;border:1px solid rgba(230,200,126,.16);border-radius:18px;background:linear-gradient(155deg,rgba(230,200,126,.06),#07080d 48%,#050609);box-shadow:0 22px 60px rgba(0,0,0,.24)}
      .mcd-hero h3{margin:0 0 .35rem}
      .mcd-hero p{margin:0;color:#c9c5b8;line-height:1.65;max-width:900px}
      .mcd-kpis{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:.75rem;margin-top:1rem}
      .mcd-kpi{padding:.95rem 1rem;border:1px solid rgba(230,200,126,.14);border-radius:14px;background:linear-gradient(145deg,rgba(230,200,126,.035),#07080d)}
      .mcd-kpi span{display:block;font-size:.78rem;color:#9b9788}
      .mcd-kpi strong{display:block;font-size:1.45rem;margin-top:.2rem;color:#f7f4ec}
      .mcd-flow{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.75rem;margin-top:1rem}
      .mcd-step{padding:.95rem;border-radius:14px;border:1px solid rgba(230,200,126,.14);background:#07080d}
      .mcd-step span{display:block;font-size:.72rem;text-transform:uppercase;letter-spacing:.08em;color:#e6c87e}
      .mcd-step strong{display:block;margin-top:.28rem}
      .mcd-step small{display:block;color:#9b9788;line-height:1.5;margin-top:.28rem}
      .mcd-main{display:grid;grid-template-columns:1.1fr .9fr;gap:1rem;margin-top:1rem}
      .mcd-bands{display:grid;gap:.8rem}
      .mcd-band{display:grid;grid-template-columns:56px minmax(0,1fr);gap:.9rem;align-items:start;padding:1rem;border:1px solid rgba(230,200,126,.14);border-radius:14px;background:linear-gradient(145deg,rgba(230,200,126,.035),#07080d);color:#f7f4ec;text-decoration:none;transition:.22s ease}
      .mcd-band:hover{border-color:#e6c87e;box-shadow:0 14px 36px rgba(0,0,0,.3);transform:translateY(-1px)}
      .mcd-band-code{width:56px;height:56px;border-radius:14px;display:grid;place-items:center;background:#050609;color:#e6c87e;font-weight:900;border:1px solid rgba(230,200,126,.14)}
      .mcd-band strong{display:block}
      .mcd-band small{display:block;margin-top:.3rem;color:#9b9788;line-height:1.55}
      .mcd-list{display:grid;gap:.75rem}
      .mcd-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.8rem;align-items:center;padding:.95rem;border:1px solid rgba(230,200,126,.14);border-radius:14px;background:#07080d;text-decoration:none;color:#f7f4ec}
      .mcd-row strong{display:block}
      .mcd-row small{display:block;color:#9b9788;line-height:1.45;margin-top:.25rem}
      .mcd-meta{display:grid;justify-items:end;gap:.3rem}
      .mcd-note{padding:.95rem;border:1px dashed rgba(230,200,126,.2);border-radius:14px;color:#9b9788;line-height:1.55;background:rgba(230,200,126,.035)}
