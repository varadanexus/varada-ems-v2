...
assert.match(adminSecrets, /\^rzp_live_/);
assert.match(adminSecrets, /const providerMode = "live"/);
assert.match(canonicalAdmin, /PAYG payment gateway · Live Mode/);
assert.match(canonicalAdmin, /\^rzp_live_/);
assert.doesNotMatch(canonicalAdmin, /data-razorpay-test-secret-form|rzp_test_/);
