...
const pwaClient = read("new-ems/shared/pwa.js");
...
assert(pwaClient.includes('navigator.serviceWorker.register("/sw.js", { scope: "/" })'), "PWA client does not register the root service worker.");
assert(pwaClient.includes('classList.add("ems-standalone")'), "PWA client must identify the installed standalone surface.");
assert(pwaClient.includes('Install Varada Nexus'), "PWA install prompts must use the Varada Nexus brand.");
assert(pwaClient.includes('className = "ems-pwa-install-dismiss"'), "PWA install prompt must provide a dedicated close control.");
assert(pwaClient.includes('sessionStorage.setItem(INSTALL_DISMISSED_KEY, "1")'), "Dismissed install prompts must stay hidden for the browser session.");
assert(liveChat.includes('id="emsChatMinimize"'), "Live chat must provide a dedicated minimize control.");
assert(liveChat.includes('getElementById("emsChatMinimize")'), "Live chat minimize control must preserve and hide the current chat state.");
